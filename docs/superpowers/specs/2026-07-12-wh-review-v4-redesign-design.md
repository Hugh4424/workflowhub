...
新增 `skills/wh-review/stage-skill-map.yaml`。它按 `stage + review_track` 选择仓库内技能、材料 profile、输出 schema、pass finding policy 和续跑 policy。技能解析只允许 `workflowhub/skills/`，不再扫描 `~/.claude/skills`、`~/.codex/skills` 或 gstack。
...
每个选择出的技能都生成 `StageSkillPlan`：logical skill id、bundle hash、允许静态文件、review mode、检查点和期望证据。可执行审查环境中才可标记为 executed；只读异源环境中的迁移技能均为 `lens-only`，不得伪造执行。
...
首轮以完整、冻结的合同、材料和技能 bundle 调用 `3rd-review` v4，保存 `initial_runtime_id`、首轮 completed provider 集、每个 provider 的业务校验结果和全部 hash。broker 自己保存 provider 原生 session；`wh-review` 不接收、不伪造、不直接传 session id。
...
`3rd-review` 增加向后兼容的通用 CLI 参数 `--attachments=<manifest.json>`，不改变 v4 request 的业务字段。ADR 同时固定 CLI 参数、退出码、stdout JSON 和错误码。manifest 由 JSON Schema 校验，包含 `version`、`bundle_id`、`entries[{source,destination,sha256,embed}]`；source 一律相对配置的 attachment root。broker 的配置列出允许 attachment roots，workflowhub 只能使用 `skills/` 与 `skills/wh-review/contracts/` 之下的源文件。首次运行时 broker：

1. 对 source 和 destination 都拒绝绝对路径、`~`、`..`、symlink、root 外文件和 hash 不匹配。
...
续跑不再接收附件，也不替换已有 bundle；broker 重新校验文件 hash 和 `skills-manifest.json`，任何不符都是 transport failure。provider capability 表分两类：A 类可读取 provider-private workspace，读取相对技能路径；B 类只接受文本 prompt，按 `embed:true` 将选定 lens 内嵌入 prompt。StageSkillPlan 必须先做 512KB prompt 预算；超限明确失败，不能静默裁剪或降级。Kimi 的 `--skills-dir` 指向该私有目录，移除其“不要使用 skills”的系统限制；Claude、Codex、OpenCode 只有能力表验证为 A 类后才可被 prompt 要求读取附件。附件功能不选择技能、不解释合同、不校验 verdict，仍是 broker 的通用受限文件传输。
...
2. `run()`：构造 prompt 和附件、调用 BrokerClient、保存 raw/diagnostic、校验每个 provider、合并为 merged result；不写 disposition。
3. 主 Agent 对 merged findings 写 `accept|reject|defer` 及证据。
4. `publish(dispositions)`：原子发布 private RoundReceipt、core receipt、中文报告、report-index 和 stage-result。
...
异常状态矩阵是 ADR 的必需附件，至少覆盖 broker timeout、attachment copy/hash failure、provider 非 JSON、business-invalid、lock 争用、disposition 超限、runtime 被 TTL 清理和 delta 不一致。每种状态固定 retry、diagnostic 和 blocked 结果。

## 7. 迁移顺序

1. 建立上述 contracts、schema、stage-skill-map、provider capability 表、legacy-rule-ledger 和附件传输 ADR；冻结旧规则的保留、迁移或删除理由。
...
- provider capability 表证明每个启用 provider 是 A 类附件读取或 B 类受限内嵌；首轮复制的 bundle 可被 A 类 provider 从私有 workspace 读取，B 类 prompt 预算不超限；续跑 hash 不变且不重拷贝。
...
- 附件边界拒绝 symlink、路径逃逸和 hash 不符；Kimi、Codex、Claude、OpenCode 的私有 workspace 行为均有 adapter 测试。
