...
function productionFiles(directory, excludedDirectories = new Set()) {
  return readdirSync(directory, { withFileTypes: true }).flatMap((entry) => {
    const path = join(directory, entry.name);
    if (entry.isDirectory()) return entry.name === "__tests__" || excludedDirectories.has(entry.name) ? [] : productionFiles(path, excludedDirectories);
...
describe("wh-review v4 workflow wiring", () => {
  ...
  it("contains no legacy review production path", () => {
    // Kimi's retired runner remains only as an isolated compatibility fixture;
    // V4 routes through wh-review-cli -> ReviewRoundFacade -> BrokerClient.
    const files = [...productionFiles(join(root, "workflows")), ...productionFiles(join(root, "skills", "wh-review"), new Set(["runners"]))];
