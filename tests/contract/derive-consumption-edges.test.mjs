...
describe("derive-consumption-edges", () => {
  ...
  it("binds a valid zero proof to one candidate snapshot and rejects reuse", () => {
    ...
    const sameProof = refreshEvolutionSnapshot({ storageRoot: root, project: "Demo", attemptId: "proof-same", now: proof.scanned_at, inventory: { observations: [item], consumer_proofs: [proof] } });
    expect(sameProof.records[0]).toMatchObject({ tier: "action_suggested", machine_signals: { zero_consumption: true } });
