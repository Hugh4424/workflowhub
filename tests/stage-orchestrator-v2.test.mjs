...
describe("stage-runner capability unit", () => {
  ...
  it("bootstraps, reads accepted upstream, invokes handlers, publishes and accepts a real v2 chain", async () => {
    ...
    await execute("build-plan", async (context, upstream) => { seen.push([context.stage, upstream.attempt.stage]); context.artifacts.writeAtomic("plan.md","plan\n"); context.artifacts.writeAtomic("tasks.md","tasks\n"); const cp=context.createCheckpoint("build-plan"), audit=(name)=>({status:"pass",result_ref:`evidence/${name}.json`,result_hash:hash}); return { facts: { plan_ref: "specs/chain-task/plan.md", tasks_ref: "specs/chain-task/tasks.md", revision:1, pair_id:"pair-1", research:audit("research"), analysis:audit("analysis"), simplicity:audit("simplicity"), review:reviewFacts("build-plan"), checkpoint: cp } }; });
