...
describe("vNext official stage completion", () => {
  ...
  it("rejects overlapping lifecycle timestamps before producing a stage outcome", () => {
    ...
    expect(() => publishCurrentWorkflowHubSession({
      context,
      input: {
        agent_run_id: "session-overlapping-lifecycle",
        session: {
          task_id: state.task.identity.taskId,
          host: "codex-desktop",
  ...
  it("fails closed when a public run has no explicit or authenticated worktree identity", () => {
    ...
    const result = spawnSync(process.execPath, [
      ...
      env: {
        ...process.env,
        HOME: home,
        CODEX_SESSION_ID: `session-missing-${process.pid}-${Date.now()}`,
        WORKFLOWHUB_TASK_DIR: storage,
      },
    });
    expect(result.status, `${result.stdout}\n${result.stderr}`).not.toBe(0);
    expect(`${result.stdout}\n${result.stderr}`).toMatch(/WorkflowHub identity missing/);
  });
