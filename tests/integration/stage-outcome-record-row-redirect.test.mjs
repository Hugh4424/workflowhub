...
describe("stage result reading is redirected to the frozen execution-record row", () => {
  ...
  it("keeps the current row authoritative when its snapshot/material fields describe older provenance", () => {
    ...
    // Stale provenance stays readable (task II D-009 removed the invalidation
    // gate) but must be disclosed instead of silently accepted.
    expect(deriveExecutionOutcomes(staleSnapshot)[STAGE].provenance).toMatchObject({
      status: "stale",
      row_snapshot_tree: "b".repeat(40),
      current_snapshot_tree: staleSnapshot.snapshot_tree,
    });
    ...
    expect(deriveExecutionOutcomes(staleMaterial)[STAGE].provenance).toMatchObject({
      status: "stale",
      row_material_revision: "c".repeat(64),
    });

    writeRow(state, { layerState: "completed" });
    const current = projectionInput(state, { readTaskFacts: recordReader(state) });
    expect(deriveExecutionOutcomes(current)[STAGE]).toMatchObject({ status: "completed" });
    expect(deriveExecutionOutcomes(current)[STAGE].provenance).toBeUndefined();
  });
...
describe("the real status entry point reads the current row", () => {
  it("reports the row-derived execution outcome end to end and follows a row-only update", () => {
    ...
    // ADR-0026: the absent current outcome is disclosed by the execution layer and must
    // not be re-published as a quality predicate.
    expect(second.quality_predicates).not.toHaveProperty("stage_outcome");
  ...
  it("keeps a row-less store honest end to end instead of inventing completion", () => {
    ...
    // The row-less store stays honest through the execution disclosure; it must not be
    // rewritten into a quality predicate or a quality `missing` entry (ADR-0026).
    expect(parsed.quality_missing).not.toContain("stage_outcome");
