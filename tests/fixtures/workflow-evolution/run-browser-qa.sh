...
node -e 'const fs=require("fs");const crypto=require("crypto");const [def,evo,narrow,errs,reqs,open,click,page,data,move,fixture,out]=process.argv.slice(1);const hash=p=>crypto.createHash("sha256").update(fs.readFileSync(p)).digest("hex");const text=[def,evo,narrow].join("\n");const expected=["Evolution","建议行动","仅供参考","前期质量税","不是质量裁决或 stage gate"];const checks={open:open==="0",evolution_tab:click==="0",content:expected.every((item)=>text.includes(item)),no_page_errors:errs.trim()===""||/no errors|No errors/i.test(errs),no_runtime_requests:!/(https?:\/\/(?!(127\.0\.0\.1|localhost)))/i.test(reqs)};const status=Object.values(checks).every(Boolean)?"passed":"qa_failed";const m={schema_version:"browser-qa-evidence.v1",status,engine:"agent-browser",login_reused:false,session:process.env.BROWSER_QA_SESSION,snapshot_sha256:crypto.createHash("sha256").update(text).digest("hex"),material_identity:{page_sha256:hash(page),data_sha256:hash(data),move_map_sha256:hash(move),fixture_sha256:hash(fixture)},assertions:[...expected,"evolution tab is reachable","390x844 and 1280x800 snapshots","no page errors","no external runtime network requests"],checks,evidence:["m16-monitor.png"],cleanup:"pending"};fs.writeFileSync(out,JSON.stringify(m,null,2)+"\n")' -- "$snapshot_default" "$snapshot_evolution" "$snapshot_narrow" "$errors" "$requests" "$open_status" "$click_status" "$html_path" "$data_path" "docs/architecture/move-map.json" "$fixture_path" "$manifest_dir/manifest.json"
kill "$server_pid" 2>/dev/null || true
node tests/fixtures/workflow-evolution/validate-browser-manifest.mjs "$manifest_dir/manifest.json"
exit $?
