...
describe("canonical step manifest", () => {
  ...
  it("keeps all five portable manifests on four materials and quality facts without legacy control-plane containers", () => {
    ...
    for (const stage of STAGES) {
      ...
      const handoffStep = item.steps.findLast((stepItem) => stepItem.step_slug !== "stage-reflection");
      expect(handoffStep?.observable_result).toMatch(/plain-language handoff|大白话交接|验收通知/i);
