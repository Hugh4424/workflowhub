...
describe("official make-decision CLI", () => {
  it("binds canonical decision receipt ref and exact byte hash into facts", () => {
    ...
    const missingDecisionLogPayload = join(root, "decision-missing-log.json"); writeFileSync(missingDecisionLogPayload, `${JSON.stringify({ content: "go", interaction_refs: ["comment:answer-1"] })}\n`);
    const incompleteDecisionPayload = join(root, "decision-incomplete.json"); writeFileSync(incompleteDecisionPayload, `${JSON.stringify({ decision_log: "# Decision\n\nGo." })}\n`);
    const fakeInteractionPayload = join(root, "decision-fake-interaction.json"); writeFileSync(fakeInteractionPayload, `${JSON.stringify({ decision_log: "# Decision\n\nGo.", interaction_refs: ["fake"] })}\n`);
    const decisionPayload = join(root, "decision.json"); writeFileSync(decisionPayload, `${JSON.stringify({ decision_log: "# Decision\n\nGo.", interaction_refs: ["comment:question-1", "comment:answer-1"] })}\n`);
    ...
    const incompleteDecision = spawnSync(process.execPath, [runtime, "receipt", "--stage=make-decision", "--project=Demo", "--task=decision-task", "--component=decision", `--input=${incompleteDecisionPayload}`], { cwd: repo, env, encoding: "utf8" });
    expect(incompleteDecision.status).not.toBe(0);
    expect(incompleteDecision.stderr).toMatch(/interaction_refs/i);
    const fakeInteraction = spawnSync(process.execPath, [runtime, "receipt", "--stage=make-decision", "--project=Demo", "--task=decision-task", "--component=decision", `--input=${fakeInteractionPayload}`], { cwd: repo, env, encoding: "utf8" });
    expect(fakeInteraction.status).not.toBe(0);
    expect(fakeInteraction.stderr).toMatch(/comment:<id>|https?/i);
    ...
    const missingConfirmationRef = spawnSync(process.execPath, [runtime, "confirm", "--stage=make-decision", "--project=Demo", "--task=decision-task", `--attempt=${result.attempt_ref}`, "--decision=accepted"], { cwd: repo, env, encoding: "utf8" });
    expect(missingConfirmationRef.status).not.toBe(0);
    expect(missingConfirmationRef.stderr).toMatch(/confirmation.*ref|comment.*id|comment.*link/i);
    const fakeConfirmationRef = spawnSync(process.execPath, [runtime, "confirm", "--stage=make-decision", "--project=Demo", "--task=decision-task", `--attempt=${result.attempt_ref}`, "--decision=accepted", "--confirmation-ref=fake"], { cwd: repo, env, encoding: "utf8" });
    expect(fakeConfirmationRef.status).not.toBe(0);
    expect(fakeConfirmationRef.stderr).toMatch(/comment:<id>|https?/i);
    const confirmation = JSON.parse(invoke(["confirm", "--stage=make-decision", "--project=Demo", "--task=decision-task", `--attempt=${result.attempt_ref}`, "--decision=accepted", "--confirmation-ref=https://multica.example/issues/ZHI-1#comment-2"]));
    expect(confirmation.confirmation.source_ref).toBe("https://multica.example/issues/ZHI-1#comment-2");
    expect(linkedWorktrees(repo)).toEqual([worktree]);
    const confirmationPath = join(task.taskPath, confirmation.ref);
    const confirmationRaw = task.readRecord(confirmation.ref);
    const confirmationWithoutSource = JSON.parse(confirmationRaw); delete confirmationWithoutSource.source_ref;
    writeFileSync(confirmationPath, `${JSON.stringify(confirmationWithoutSource, null, 2)}\n`);
    const missingSourceAtAccept = spawnSync(process.execPath, [runtime, "accept", "--stage=make-decision", "--project=Demo", "--task=decision-task", `--attempt=${result.attempt_ref}`, `--human-confirmation-ref=${confirmation.ref}`], { cwd: repo, env, encoding: "utf8" });
    expect(missingSourceAtAccept.status).not.toBe(0);
    expect(missingSourceAtAccept.stderr).toMatch(/comment:<id>|https?/i);
    confirmationWithoutSource.source_ref = "fake";
    writeFileSync(confirmationPath, `${JSON.stringify(confirmationWithoutSource, null, 2)}\n`);
    const fakeSourceAtAccept = spawnSync(process.execPath, [runtime, "accept", "--stage=make-decision", "--project=Demo", "--task=decision-task", `--attempt=${result.attempt_ref}`, `--human-confirmation-ref=${confirmation.ref}`], { cwd: repo, env, encoding: "utf8" });
    expect(fakeSourceAtAccept.status).not.toBe(0);
    expect(fakeSourceAtAccept.stderr).toMatch(/comment:<id>|https?/i);
    writeFileSync(confirmationPath, confirmationRaw);
  ...
  it("binds full grill-with-docs writes to the published candidate snapshot", () => {
    ...
    const confirmation = JSON.parse(invoke(["confirm", "--stage=make-decision", "--project=Demo", "--task=grill-task", `--attempt=${result.attempt_ref}`, "--decision=accepted", "--confirmation-ref=comment:final-direction"]));
