...
describe("per-invocation documentation contract", () => {
  it("keeps constitution and checklist aligned on execution identity", () => {
    ...
    const versionMatches = [...constitution.matchAll(/^Version: (\d+\.\d+\.\d+)/gm)].map(([, version]) => version);
    expect(versionMatches).toHaveLength(2);
    expect(versionMatches[0]).toBe(versionMatches[1]);
    expect(checklist).toContain(`Version ${versionMatches[0]}`);
