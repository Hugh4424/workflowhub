...
/** Opt-in live proof that the current audit scope reaches OpenCode as file-only material. */
...
export function assertAuditEvidence({ receipt, packet, auditScopeFiles, markers, evidenceSources, providerIds = ["opencode"] }) {
  ...
  for (const provider of providerIds) {
    ...
    requireValue(outcome.delivery?.delivery_mode === "file_only" && outcome.delivery?.byte_identity === "verified"
      && /^[a-f0-9]{64}$/.test(outcome.delivery?.sealed_manifest_hash ?? "")
      && outcome.delivery.provider_visible_manifest_hash === outcome.delivery.sealed_manifest_hash
      && outcome.delivery.sealed_manifest_hash === receipt.delivery?.material_manifest_hash, `AUDIT_E2E_${provider}_EXACT_COPY_RECEIPT_INVALID`);
    const visible = new Map((outcome.delivery?.provider_visible_attachment_manifest ?? []).map((item) => [item.destination, item]));
    requireValue(visible.get("changes.diff")?.sha256 === packet.diff_sha256 && visible.has("review-packet.v1.json") && visible.has("manifest.json"), `AUDIT_E2E_${provider}_PACKET_HASH_MISMATCH`);
    for (const source of evidenceSources) {
      const item = visible.get(`evidence/${source}`);
      requireValue(/^[a-f0-9]{64}$/.test(item?.sha256 ?? "") && Number.isSafeInteger(item?.size) && item.size > 0, `AUDIT_E2E_${provider}_EVIDENCE_MANIFEST_INVALID: ${source}`);
    }
    ...
    evidence[provider] = { session_id: outcome.session_id, raw_stdout_ref: outcome.raw_stdout_ref, raw_stdout_sha256: outcome.raw_stdout_sha256, sealed_manifest_hash: outcome.delivery.sealed_manifest_hash };
  }
  return evidence;
}
...
async function main() {
  ...
  try {
    ...
    for (const [index, [name, marker]] of [["000-review-e2e-begin.txt", markers[0]], ["specs/m14a-audit-contract-layer/m-review-e2e-middle.txt", markers[1]], ["zzz-review-e2e-end.txt", markers[2]]].entries()) writeFileSync(join(mirror, name), `${marker}\n${index === 1 ? "M".repeat(4096) : ""}`);
    ...
    writeFileSync(join(tracking, taskId, "test-strategy.md"), "# Audit E2E test strategy\n\nThe temporary mirror tree must match the live audit tree before marker injection. OpenCode must read all three markers from the frozen file-only packet.\n");
    const evidenceSources = ["test-strategy.md", "evidence/phase-1-GREEN.json", "evidence/phase-1-diff-scan.json"];
    for (const source of evidenceSources.slice(1)) { const destination = join(tracking, taskId, source); mkdirSync(dirname(destination), { recursive: true }); copyFileSync(join(options.task, source), destination); }
    const resolution = resolveRequiredSkills({ stage: "build-code", reviewTrack: null });
    const input = { task_id: taskId, stage: "build-code", review_flow_id: "full-scope", host_provider: "codex", provider_allowlist: ["opencode"], task_tracking_root: tracking, packet: { version: "review-packet.v1", stage: "build-code", review_track: null, round_kind: "initial", baseline_packet_hash: null, raw_requirement: `Review the complete audit scope and quote ${markers.join(", ")} from changes.diff.`, acceptance_design_excerpt: "The complete host-captured audit diff and all three marker positions must be reviewed.", test_evidence: evidenceSources.map((source, index) => ({ fact_id: `audit-e2e-artifact-${index + 1}`, kind: "artifact", source, captured_at: new Date().toISOString(), sha256: hash(readFileSync(join(tracking, taskId, source))), status: "passed" })), host_verified_facts: [{ fact_id: "audit-scope", kind: "source-tree", source: "host reconciliation", captured_at: new Date().toISOString(), sha256: hash(auditTree), value: { reconciled_files: auditMaterial.changed_files.length } }], contract_hash: projectStageContract("build-code").contractHash, skill_bundle_hash: resolution.skillBundleHash }, dispositions: { items: [] } };
    ...
    const providers = assertAuditEvidence({ receipt, packet, auditScopeFiles: auditMaterial.changed_files.length, markers, evidenceSources });
    requireValue(packet.source_revision.snapshot_tree === deliveryTree, "AUDIT_E2E_PACKET_TREE_MISMATCH");
    const verifyInputPath = join(options.output, "verify-final-input.json"); writeJson(verifyInputPath, { task_id: taskId, stage: "build-code", review_flow_id: "full-scope", task_tracking_root: tracking });
    const verify = spawnSync(process.execPath, [join(repository, "skills/wh-review/scripts/wh-review-cli.mjs"), "verify-final", verifyInputPath], { encoding: "utf8", env: hostOverride.env ?? process.env });
    writeJson(join(options.output, "verify-final-execution.json"), { status: verify.status, stdout: verify.stdout, stderr: verify.stderr });
    requireValue(verify.status === 0, `AUDIT_E2E_VERIFY_FINAL_FAILED: ${verify.stderr.trim()}`);
    const verifyFinal = JSON.parse(verify.stdout); requireValue(verifyFinal.finalized === true && verifyFinal.approved_tree === deliveryTree, "AUDIT_E2E_VERIFY_FINAL_TREE_MISMATCH");
    const evidence = { status: "PASS", approved_base_commit: baseCommit, approved_base_tree: baseTree, audit_captured_tree: auditTree, delivered_tree: deliveryTree, audit_scope_files: auditMaterial.changed_files.length, delivered_files: packet.changed_files.length, copied_untracked: untracked, packet_hash: packet.packet_hash, diff_sha256: packet.diff_sha256, runtime_id: receipt.runtime_id, providers, verify_final: verifyFinal, receipt: receiptPath, packet: packetPath, input: inputPath, verify_input: verifyInputPath };
