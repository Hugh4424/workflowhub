...
export function findUndeclaredStaticDependencies({ skillDir, fileEntries }) {
  ...
  for (const entry of fileEntries) {
    ...
    if (!MARKDOWN_EXTENSIONS.has(extension) && !CODE_EXTENSIONS.has(extension)) continue;
    ...
    for (const locator of references(content, extension)) {
      ...
      if (!fs.existsSync(target)) {
        if (CODE_EXTENSIONS.has(extension)) missing.push({ source: entry.path, locator, reason: "referenced file is missing" });
