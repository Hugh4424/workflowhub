...
HANDLERS.set("build-plan", async (worker, input) => { const plan = receipt(worker, input, "plan"), tasks = receipt(worker, input, "tasks"), review = reviewFacts(worker, input); text(plan.value.content, "plan content"); text(tasks.value.content, "tasks content"); if (plan.value.content_hash !== hashText(plan.value.content) || tasks.value.content_hash !== hashText(tasks.value.content)) throw new Error("plan/tasks content hash mismatch"); worker.writeArtifact("plan.md", plan.value.content); worker.writeArtifact("tasks.md", tasks.value.content); const checkpoint = worker.createCheckpoint("build-plan"); if (review.facts.snapshot_tree !== worker.snapshotWorkspace().tree) throw new Error("plan review and Workspace must bind the same snapshot tree"); return { facts: { plan_ref: worker.artifactRef("plan.md"), tasks_ref: worker.artifactRef("tasks.md"), checkpoint, review: review.facts }, evidence_refs: [plan.evidence, tasks.evidence, review.evidence], missing_items: review.missing_items }; });
