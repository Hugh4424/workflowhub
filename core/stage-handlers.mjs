...
function verifyReviewChain(worker, result, expectedTrack, producerStage = worker.stage) {
  ...
  const minimumReviewers = minimumReviewersFor(producerStage, expectedTrack ?? null);
...
function verifyUnavailableReview(worker, item, expectedTrack, producerStage = worker.stage) {
  ...
  const aggregation = aggregateProviderResults(recomputed, minimumReviewersFor(producerStage, expectedTrack ?? null));
...
HANDLERS.set("verify-code", async (worker, input) => {
  ...
  const result = {
    ...
    missing_items: [...review.missing_items, ...mismatches, ...(failedEvidence.length ? failedEvidence.map((entry) => `failed acceptance evidence: ${entry.ref}`) : [])],
  };
