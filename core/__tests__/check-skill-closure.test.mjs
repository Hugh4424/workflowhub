...
function fixture({
  ...
  fs.writeFileSync(path.join(root, `workflows/${stage}/skill-deps.yaml`), manifestSkill
    ? `stage: ${stage}\nskills:\n  - { name: ${skillName}, path: skills/${skillName}/SKILL.md, execution: inline, trigger: always, bundle: skills/${skillName}/skill-bundle.json, owner: stage, consumer: { target: stage-handlers#testFacts, inputs: [receipts.tests], identity: [task_id, stage, workspace_path], result: facts.tests } }\nruntime_capabilities: []\nexternal_capabilities: []\n`
    : `stage: ${stage}\nskills: []\nruntime_capabilities: []\nexternal_capabilities: []\n`);
  if (indirectLens) {
    ...
    fs.writeFileSync(path.join(root, "workflows/stage/skill-deps.yaml"), "stage: stage\nskills:\n  - { name: wh-review, path: skills/wh-review/SKILL.md, execution: inline, trigger: review, bundle: skills/wh-review/skill-bundle.json, owner: stage, consumer: { target: stage-handlers#safeReviewFacts, inputs: [receipts.review], identity: [task_id, stage, workspace_path], result: facts.review } }\nruntime_capabilities: []\nexternal_capabilities: []\n");
  }
  return root;
}
