...
describe("runtime cutover authority",()=>{
  ...
  it("stage-runtime CLI bootstraps through the same runtime authority",()=>{const f=fixture(),active=assertRuntimeAuthority(f.a,{home:f.home});quiesceRuntime({storageRoot:f.a,home:f.home,expectedEpoch:active.cutover_epoch});const child=spawnSync(process.execPath,[join(process.cwd(),"scripts","stage-runtime.mjs"),"run","--action=execute","--stage=build-spec","--project=Demo","--task=blocked","--input=missing.json"],{encoding:"utf8",env:{...process.env,HOME:f.home,WORKFLOWHUB_TASK_DIR:f.a}});expect(child.status).not.toBe(0);expect(child.stderr).toMatch(/quiescing|refused/i);expect(child.stderr).not.toMatch(/ENOENT.*missing\.json/i);});
