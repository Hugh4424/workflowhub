...
**UC-8 失败场景：关键输入缺失或结果不可解析或 runner 调用失败（关联 FR-WHREVIEW-001/002/003/THIRDREVIEW-001、NFR-2；本轮修复：round30 曾因 3rd-review runner 调用挂起卡死无法自动恢复，Given/Then 补齐"runner 不存在/不可执行/非零退出/调用超时"三类此前未在本 UC 显式枚举、但 FR-THIRDREVIEW-001"不可达/失败语义"早已定义的失败条件，避免仅靠"result-file 缺失/不可解析"泛化表述掩盖 runner 调用超时这一独立触发条件）**
Given stage 标识缺失或为未知值，或 stage 对应的合同文件缺失，或 3rd-review runner 不存在/不可执行/非零退出/调用超时，或 3rd-review 返回的 result-file 缺失/不可解析；
When wh-review 执行到对应校验点；
Then 按场景分别处理，不静默降级、不回退通用合同、不跳过校验继续推进：stage 标识缺失/未知 → 属于 wh-review 自身输入/配置错误，以非零退出码终止并输出明确错误信息（fail-loud，NFR-2）；stage 对应合同文件缺失 → 同样属于 wh-review 自身配置错误，以非零退出码终止并输出明确错误信息（fail-loud，NFR-2）；3rd-review runner 不存在/不可执行/非零退出/调用超时，或返回的 result-file 缺失/不可解析 → 均属于 3rd-review 调用失败场景（不是 wh-review 自身故障），最终裁决直接判定为 `escalate_to_human`，`failure_reason` 按实际触发原因记为 `runner-missing`/`non-zero-exit`/`timeout`/`output-unparseable` 之一并写入报告/日志，wh-review 自身以退出码 0 正常返回（NFR-2 明确的例外条款，不适用 fail-loud 非零退出规则，权威定义见 FR-THIRDREVIEW-001/NFR-2，验收标准见 AC5-5）。
...
**路径安全约束（round23 修复，消除路径穿越风险；round27 修复：由"wh-review 自身遵守"扩展为"所有基于 task_id 的落盘操作前置条件"，堵住 FR-WHREVIEW-007 步骤2 提示词生成子代理绕过该校验先行写盘的缺口）**：`task_id` 参数值仅允许安全字符集 `^[A-Za-z0-9._-]+$`（不含 `/`、`\` 等路径分隔符，不含 `..`）；**该校验不是 wh-review 一方的自我约束，而是任何一方——wh-review 自身、stage 主 agent、FR-WHREVIEW-007 定义的审查提示词生成子代理等——只要要基于 `task_id` 拼接落盘路径（如上述 `path.join(task_tracking_root, "tasks", task_id, ...)`），都必须先在自己的写入代码路径中执行同一套字符集校验**；不匹配时须 fail-loud（非零退出+明确错误信息）报错并终止，不得对该值做静默清洗（如剥离非法字符）或截断后继续使用，以防带 `../`、路径分隔符或绝对路径片段的 `task_id` 突破任务目录边界写错位置。**不允许"先由某一方写入落盘文件、之后再由 wh-review 事后校验"这种时序**——校验必须发生在自身的写入操作执行之前，不得把该责任下放或延后到调用链中的其他角色身上（如子代理先落盘、等 wh-review 下一次调用才校验），否则路径穿越写入已在校验发生前实际完成，校验形同虚设。
...
- AC1-6（fail-loud，round23 新增，task_id 路径安全约束）：给定调用方传入的 `task_id` 参数值不匹配安全字符集 `^[A-Za-z0-9._-]+$`（如包含 `../`、`/`、`\` 或其他路径分隔符），wh-review 在执行任何基于该值的落盘路径拼接操作前即以非零退出码报错终止，不得静默清洗（如剥离非法字符）或截断该值后继续使用；给定合法字符集内的 `task_id`，wh-review 正常完成落盘操作。断言：非法 `task_id` 场景 exit code ≠ 0 且未在文件系统上产生任何越界写入（可用临时目录做前后对比验证）；合法场景落盘路径中的 `task_id` 片段与入参值一致。
...
**落盘路径（定死，权威声明，AC3-1~AC3-4 及 §6/AC-D10/AC-D10.1 验收依据；round21 修复：路径按 `stage`+`review_flow_id` 两个维度隔离，不再是覆盖整个 task 的单一全局文件，与 FR-WHREVIEW-002 定义的 `route-decision-{stage}-{review_flow_id}.json` 采用相同隔离方式，理由与发现规则见 data-contracts.md Contract 4"跨 stage/跨流程隔离结论"——不再依赖 FR-STAGE-001 未定义的"stage 严格串行"假设）**：轮次状态文件固定落盘于 `tasks/{task-id}/reviews/round-state-{stage}-{review_flow_id}.json`（`task-id` 来源见 FR-WHREVIEW-001"task-id 来源契约"；与 FR-WHREVIEW-002 定义的 `route-decision-{stage}-{review_flow_id}.json` 共享同一 `tasks/{task-id}/reviews/` 目录结构，不另设新目录）。plan.md/tasks.md/data-contracts.md 中所有对轮次状态文件路径的引用均以本条款为唯一权威来源，不得另行定义或改写。
...
- `review_flow_id`（本轮修复新增，权威声明，消除"不同审查流程之间因 `total_round` 复位而产生文件名撞车"的缺口，round14 finding）：字符串，标识"本次审查流程"的稳定唯一 ID，仅含 `[a-z0-9-]` 字符（避免破坏文件名解析）。生成时机：某 stage 发起一次全新审查流程时（即该 stage 当前没有处于进行中的流程、`total_round` 即将从1重新计数时）由 wh-review 生成一次，推荐格式 `{十进制毫秒时间戳}-{4位随机十六进制}`（如 `1770412345678-a1b2`），具体生成算法由实现自行选择，只需保证同一 `task-id` 下先后发起的不同审查流程之间该值不重复。**并发约束（定死，round24 修复；round25 修复：收紧"无进行中流程"判定，堵住 D2 人工确认门被绕过的缺口，权威定义见 data-contracts.md Contract 4 附属"活跃审查流程指针文件"）**：判定"当前没有处于进行中的流程"须先读取 `tasks/{task-id}/reviews/active-flow-{stage}.json` 指针文件（若存在），取得既有 `review_flow_id` 后定位 `round-state-{stage}-{既有review_flow_id}.json`——该文件不存在，或存在但已满足"流程已结束"判定（`verdict=escalate_to_human`；或 `verdict=pass` 且 `post_review_action=auto_advance`；或 `verdict=pass` 且 `post_review_action=await_human_confirmation` 且对应人工确认 artifact `human-confirmation-{stage}-{既有review_flow_id}-{total_round}.json` 已存在且其 `stage`/`review_flow_id`/`total_round` 字段与当前 round-state 一致，字段定义见 FR-D2-001）时，才判定为无进行中流程，可分配全新值；`verdict=pass` 但 `post_review_action=await_human_confirmation` 且上述人工确认 artifact 尚不存在或字段不匹配时，一律仍视为进行中，不得判定为无进行中流程（round25 finding：否则会让新的 prepare 调用绕过尚未完成的 D2 人工确认门，分配新 `review_flow_id`）；若该文件存在且未满足"流程已结束"判定（进行中），prepare 步骤（FR-WHREVIEW-007 步骤1）默认须**复用**该既有 `review_flow_id`，不得分配新值——这是正常的"同一审查流程继续下一轮"或"pass 但等待人工确认中"场景，不算并发冲突，也不得因此绕过 D2 门；只有调用方明确要求"强制开新流"时，才允许在既有流程仍进行中的情况下覆盖 `active-flow-{stage}.json` 指针，且覆盖前必须 fail-loud 报错、要求显式确认后才可继续，不得静默覆盖一个尚在进行中的流程（round24 finding：同一 `task_id`+`stage` 若出现并发重跑/重入，最后一次 prepare 覆盖指针会导致重启恢复拿错 flow）。生命周期：同一审查流程内（从发起到得出 `pass`/`escalate_to_human` 终态为止，含异源转同源的模式切换）该值保持不变，不因轮次递增或模式切换而重新生成；下一次全新审查流程发起时须重新生成一个新值。用途：报告文件名（FR-WHREVIEW-001 命名规则）、`prompt-r{N}.md`（FR-WHREVIEW-007）、人工确认 artifact（FR-D2-001）、raw verdict 证据文件（FR-THIRDREVIEW-001）、文档快照文件（FR-WHREVIEW-006）五类跨轮持久化 artifact 的文件名均须纳入该字段，以在 `total_round` 复位场景下与历史流程的同名文件区分，避免覆盖历史证据。
- `stage`：触发本次审查的 stage 标识，取值 make-decision/build-spec/build-plan/build-code/verify-code 之一，由触发方在每次写入时显式提供，不得留空、不得由 wh-review 自行猜测。orchestrator 恢复/重启读取该文件前，须先校验该字段与调用方自身当前 stage 一致；不一致须 fail-loud 报错并终止，不得静默假定该文件属于当前 stage 继续消费（round21 修复：本文件路径已按 `stage`+`review_flow_id` 隔离，orchestrator 若尚不持有 `review_flow_id`，须先读取 `tasks/{task-id}/reviews/active-flow-{stage}.json` 指针文件取得 `review_flow_id`，据此直接定位对应文件，定位后仍按本条款校验其内部 `stage`/`review_flow_id` 字段与指针/文件名一致作为二次核验，发现规则权威定义见 data-contracts.md Contract 4 附属"活跃审查流程指针文件"；round22 修复：不再以 mtime 通配排序作为默认发现路径，mtime 仅作指针文件丢失/损坏时的人工兜底排障手段，见 data-contracts.md Contract 4"发现规则"段）。
...
- `finding_fingerprints`（本轮新增，追踪粒度细化到单条 finding，字段全集类型定义见 data-contracts.md Contract 4；**round28 修复：覆盖范围由"仅追踪最终仍判为 blocking 的 finding"扩展为"追踪本轮 3rd-review 原始返回、进入 FR-WHREVIEW-005 降级判定范围的全部 blocking finding，含被该规则默认降级为 minor 的 finding"**——若被默认降级为 minor 的 finding 不写入本数组，下一轮 FR-WHREVIEW-005 判定"该指纹是否曾出现过"时会读不到历史记录，误判为"从未出现过的真正新发现"，导致同一 finding 每轮都被当作新发现重复默认降级，永远不会因"曾出现过"而维持原判定，round28 finding）：对象数组，元素结构 `{finding_fingerprint: string, first_seen_round: integer, consecutive_unresolved_rounds: integer, last_status: "open"|"resolved", diagnosed: boolean, severity_decision: string|null}`。`finding_fingerprint` 为该 finding 的稳定指纹，基于其定位点（`file`+`line` 或等效定位锚点）与问题类别（`category`）做 hash 得出，与措辞变化无关；每轮审查完成后，wh-review 须将本轮 3rd-review 原始返回的全部 blocking finding（即 FR-WHREVIEW-005 降级判定之前的集合，含最终维持 blocking 与被默认降级为 minor 两类）逐一计算指纹并与已有条目比对：指纹存在且本轮最终判定仍为 blocking → `consecutive_unresolved_rounds` 递增、`last_status="open"`；指纹存在但本轮最终判定已不属于 blocking（含被默认降级为 minor、或已闭合不再出现两种情形）→ `last_status="resolved"`，`consecutive_unresolved_rounds` 归零；指纹不存在 → 新增条目、`consecutive_unresolved_rounds=1`。该数组只追加/更新已有条目，不删除历史指纹记录；条目当前"是维持 blocking 还是被降级为 minor"由同一条目的 `severity_decision` 字段区分（枚举定义见 data-contracts.md Contract 4，判定规则见下方 FR-WHREVIEW-005），不新增独立历史表。
...
- **升级人工触发条件（精确定义，round14 修复：拆分为轮级粗粒度信号与 finding 级细粒度信号两类，避免"整轮 blocking 指纹集合是否完全相同"这一粗粒度判据掩盖单条 finding 反复出现、其余 findings 已变化的情况；round16 修复：两类信号的"连续N轮"阈值不再共用同一个 N——轮级信号仍为连续3轮，finding 级信号阈值降为连续2轮）**："连续N轮"在异源阶段指连续N个 `heterologous_round`，在同源阶段指连续N个 `same_source_round`，两类计数器分别独立比较，不跨阶段合并；轮级信号 N=3，finding 级信号 N=2（阈值不同的原因见下方 finding 级信号段"round16 修复"说明）：
  ...
  - **finding 级信号（round14 修复替代原"`fingerprint_repeated = true` 连续3轮直接升级人工"判据；round16 二次修复：触发阈值从3降为2；round26 三次修复：恢复"轮次硬顶感知"判定，订正 round16 被证伪的"阈值改为2后必有下一轮"论证）**：当 `finding_fingerprints`（见上方"字段完整性、历史记录与自校验"）中某条记录的 `consecutive_unresolved_rounds` 在第 `R` 轮（`R` 取该 finding 所在阶段计数器——异源阶段为 `heterologous_round`、同源阶段为 `same_source_round`——当轮取值）达到2（即同一条 blocking finding 的指纹连续2轮 `last_status="open"`）时，须同时判断 `R` 是否已等于该 finding 所在阶段的轮次硬顶（异源硬顶3轮/同源硬顶3轮，二者独立判断）：
    - **`R < 该阶段轮次硬顶`（下一轮仍在硬顶内）**：wh-review **不得**直接裁决 `escalate_to_human`；须先触发"根因诊断"步骤：要求诊断该 finding 反复出现的根本原因（`subsystem_design_defect`〈同一子系统设计缺陷〉或 `prior_fix_direction_wrong`〈历轮修复方向本身错误〉二者之一，或标注 `other` 并注明具体原因），把诊断结论追加写入 `root_cause_diagnoses` 数组（含触发轮次 `triggered_round=R`），并针对该根因发起一次定向修复尝试（对应记录 `fix_attempt_round=R+1`，即修复尝试提交后触发的下一轮审查轮次；`R+1` 恒不超过该阶段轮次硬顶，必有空间执行）；仅当该修复尝试对应轮次（第 `R+1` 轮）审查完成后，该 `finding_fingerprint` 在 `finding_fingerprints` 中仍为 `last_status="open"`（即诊断+修复尝试后仍未解决）时，方裁决 `escalate_to_human`，并将 `root_cause_diagnoses` 对应条目的 `resolved` 置为 `false`；若修复尝试后该 finding 已闭合（`last_status="resolved"`），则该条目 `resolved` 置为 `true`，不触发 `escalate_to_human`，流程按其余 findings 情况继续正常判定降级/升级。
    - **`R = 该阶段轮次硬顶`（已无下一轮可供修复尝试）**：直接裁决 `escalate_to_human`，跳过"根因诊断+定向修复尝试"步骤，不追加 `root_cause_diagnoses` 记录（阶段内已无剩余轮次可供尝试修复，诊断/重试步骤没有意义）。

    `fingerprint_repeated`（轮级、整轮指纹集合是否完全相同的布尔字段，定义见 Contract 4）自 round14 起不再直接触发升级人工，仅作报告/排查用的粗粒度参考信号。**round26 修复：订正 round16"删除优先级例外条款"时给出的已被证伪的论证**——round16 曾论证"阈值降为2后，某 finding 首次达到 `consecutive_unresolved_rounds=2` 必发生在阶段第1或第2轮末，第3轮硬顶内必有空间完成修复尝试，不再存在'终审轮无下一轮'的场景"，因而删除了 round14 引入的"终审轮无下一轮，跳过诊断直接升级"例外分支；该论证隐含的前提是"该 finding 必然从阶段第1轮起就已出现"，但若某 finding 直到该阶段第2轮才首次出现（`first_seen_round` 为该阶段第2轮），其 `consecutive_unresolved_rounds` 达到2 只会发生在该阶段第3轮末——此时第3轮本身就是阶段轮次硬顶，已无第4轮可供修复尝试，round16 的论证在此场景下不成立（round26 finding）。本轮不再依赖"阈值改为2后必有下一轮"这一假设本身，而是显式引入上方"`R` 是否等于阶段轮次硬顶"的判断分支：无论该 finding 是阶段第1轮还是第2轮才首次出现，只要达到阈值时判定出 `R < 硬顶`，就走"诊断+下一轮重试"路径；判定出 `R = 硬顶`，就直接跳过诊断、升级人工——两分支合起来覆盖全部情形，不再存在"规则要求的下一轮不存在"的断裂。
...
- **优先级规则**：每轮末先判升级人工条件，满足则直接升级，不再进入下一轮同源切换；不满足升级条件时，再按轮次判断是否转同源。即：`heterologous_round = 3` 末如满足升级条件，直接升级人工，`same_source_round` 不启动；不满足时才切换到同源模式，`same_source_round` 从1开始计数。**round16 修复，round26 订正**：阈值改为2后，finding 级信号与本条"阶段终审轮升级"配合处理如下——若某 finding 达到 `consecutive_unresolved_rounds=2` 时 `R < 该阶段轮次硬顶`（多发生在阶段第1或第2轮末），触发诊断+下一轮定向修复尝试，修复尝试对应轮次若恰为该阶段终审轮（第3轮）且尝试后仍未解决，finding 级信号自身"仍未解决则升级"的结论与阶段终审轮硬顶的强制升级结论天然一致，直接按 finding 级信号规则裁决 `escalate_to_human` 即可；**round26 修复**：若该 finding 直到阶段第2轮才首次出现，达到 `consecutive_unresolved_rounds=2` 时 `R` 本身就等于该阶段轮次硬顶（第3轮），按上方"finding 级信号"`R=硬顶` 分支，直接跳过诊断步骤裁决 `escalate_to_human`，与本条"每轮末先判升级人工条件，满足则直接升级"的优先级规则天然一致，两种起点场景均不需要额外的"优先级例外"条款。
- **同源模式终止规则**：同源审查最多3轮（`same_source_round` 上限为3，独立计数，不与 `heterologous_round` 合并）；`same_source_round = 3` 末若仍非 pass，强制裁决 `escalate_to_human`，不允许无限循环。同源模式下轮级/finding 级升级触发条件与异源相同（定义见上方"升级人工触发条件"），但因 `same_source_round` 上限为3，实际触发时机为：`same_source_round = 3` 末仍非 pass → 直接升级人工，无需等待3轮连续条件；此强制升级不因某条 finding 的根因诊断+修复尝试仍在进行中而豁免——**round16 修复，round26 订正**：若该 finding 在 `same_source_round≤1` 时已出现并在第2轮末达到 `consecutive_unresolved_rounds=2`（`R=2 < 硬顶3`），诊断+修复尝试最迟在 `same_source_round=3`（硬顶）完成，若该轮仍未解决，finding 级信号自身规则与本条终止规则结论一致，均为升级人工；**round26 修复**：若该 finding 直到 `same_source_round=2` 才首次出现、在 `same_source_round=3`（硬顶）末才达到 `consecutive_unresolved_rounds=2`（`R=3=硬顶`），按"finding 级信号"`R=硬顶` 分支直接跳过诊断步骤裁决 `escalate_to_human`，同样与本条终止规则结论一致——两种起点场景均不存在需要豁免的冲突场景，不需要恢复"优先级例外"条款。**这三个计数器均由 wh-review 自身独立维护、落盘在其轮次状态文件中，不依赖、也不透传给 3rd-review 引擎内部的任何轮次参数或字段——3rd-review 引擎调用入口对 round/stage 保持零知识（见 FR-THIRDREVIEW-001），本条款的三计数器是 wh-review 侧的调用方状态，不改变该零知识约定（见 GAP-6）。**
...
- AC3-5（行为验证，本轮新增；round22 新增第④项）：①给定连续3轮均满足单轮升级条件的模拟场景，断言升级判定逻辑实际读取轮次状态文件的 `history` 数组中最近3条同类型（`heterologous`/`same-source`）快照逐一核验，而非仅使用覆盖后的顶层单一快照字段；②给定轮次状态文件 `history` 数组中同类型快照不足3条，断言不满足升级条件（数据不足不得误判为满足）；③给定恢复读取轮次状态文件时其 `stage` 字段与调用方当前 stage 不一致，断言 wh-review/orchestrator fail-loud 报错退出，不采用该文件内容继续推进；④给定 orchestrator 恢复/重启时按 `active-flow-{stage}.json` 指针文件取得的 `review_flow_id` 与定位到的 `round-state-{stage}-{review_flow_id}.json` 文件内部 `review_flow_id` 字段或文件名不一致，断言 fail-loud 报错退出，不静默采用任一份继续推进（round22 修复，见 data-contracts.md Contract 4 附属"活跃审查流程指针文件"）。
- AC3-6（行为验证，round14 新增，finding 级指纹追踪 + 根因诊断改判；round16 修复：阈值3改为2，删除⑤"优先级例外"验证点；round26 修复：恢复"轮次硬顶感知"判定，新增⑥验证 `R=硬顶` 直接升级分支）：①给定某条 blocking finding 的 `finding_fingerprint` 在 `finding_fingerprints` 中连续2轮 `last_status="open"`（`consecutive_unresolved_rounds=2`）且当轮 `R < 该阶段轮次硬顶` 的模拟场景，断言 wh-review 在该轮末**不**直接裁决 `escalate_to_human`，而是向 `root_cause_diagnoses` 追加一条含 `diagnosis`/`category`/`triggered_round` 的记录，并驱动下一轮审查作为该根因的定向修复尝试（`fix_attempt_round` 对应该下一轮）；②给定修复尝试对应轮次审查完成后该 `finding_fingerprint` 在 `finding_fingerprints` 中转为 `last_status="resolved"`，断言裁决字段**不**为 `escalate_to_human`，且对应 `root_cause_diagnoses` 条目 `resolved=true`；③给定修复尝试对应轮次审查完成后该 `finding_fingerprint` 仍为 `last_status="open"`，断言裁决字段 = `escalate_to_human`，且对应 `root_cause_diagnoses` 条目 `resolved=false`；④断言 `fingerprint_repeated=true` 单独出现（未达到 finding 级连续2轮条件）时不触发 `escalate_to_human`，验证该字段已不再是直接升级判据；⑤（round16 修复，替换原"优先级例外"行为验证，验证诊断+重试路径在阶段轮次硬顶内可达）给定某 finding 在 `heterologous_round=1` 首次出现并判为 blocking、`heterologous_round=2` 末仍为 blocking（`consecutive_unresolved_rounds=2`，`R=2 < 硬顶3`，触发①所述诊断+定向修复尝试，`fix_attempt_round=3`）、`heterologous_round=3`（该阶段轮次硬顶，仍在硬顶内）为修复尝试对应审查轮次的模拟场景，断言：若该轮该 finding 转为 `last_status="resolved"`，裁决字段不为 `escalate_to_human`（对应②）；若该轮仍为 `last_status="open"`，裁决字段 = `escalate_to_human`（对应③）——两种结果均在 `heterologous_round ≤ 3` 硬顶内产生，验证"诊断→下一轮重试→仍不行才升级"路径在阶段轮次硬顶内真实可达；⑥（round26 新增，验证 `R=硬顶` 分支：finding 第2轮才首次出现的场景）给定某 finding 在 `heterologous_round=2` 首次出现并判为 blocking、`heterologous_round=3`（该阶段轮次硬顶）末仍为 blocking（`consecutive_unresolved_rounds=2`，此时 `R=3` 恰等于阶段轮次硬顶）的模拟场景，断言 wh-review **跳过**"根因诊断+定向修复尝试"步骤（不追加 `root_cause_diagnoses` 记录），直接裁决 `escalate_to_human`；same-source 阶段同理，给定某 finding 在 `same_source_round=2` 首次出现并判为 blocking、`same_source_round=3`（硬顶）末仍为 blocking，断言同样直接跳过诊断步骤裁决 `escalate_to_human`，验证无论 finding 第1轮还是第2轮才首次出现，均不存在"规则要求的下一轮不存在"的断裂。
...
**描述**：round7-13 审查反复未收敛的根因调研发现，新方案缺了 agenthub 原版真正让审查3轮收敛的核心机制之一——round2+ 审查中新出现的 blocking finding，默认应降级为 minor（不阻断 pass），仅特定例外仍算 blocking。**round24 修复：判定主体改为 wh-review 本地计算，不下放给 3rd-review**——3rd-review 引擎调用边界（Contract 1）仅接受 `{mode, contract, materials}` 三个字段，不透传 stage/round 历史（`finding_fingerprints`/`root_cause_diagnoses`/finding 已闭合状态等），文档类场景下 `materials` 更收紧为快照 diff，3rd-review 物理上无法执行本 FR 定义的跨轮历史比对（round24 finding）。本 FR 把该判据显式定义为硬性条款，由 wh-review 在收到 3rd-review 返回的原始 `{verdict, findings, actual_mode}` 后，在自身逻辑（`skills/wh-review/scripts/round-state.mjs`，见 tasks.md T011）里本地完成判定与 severity 改写，使裁决可落地执行；不要求、也不下放给 3rd-review 调用所依据的各 stage 专属合同（FR-WHREVIEW-002）——3rd-review 保持零 stage/round 历史知识（见 FR-THIRDREVIEW-001），只需按现有 findings 结构返回携带确定性定位信息（`file`/`line`/`category`）的 findings，供 wh-review 自行计算 `finding_fingerprint` 并与本地历史比对。

**规则定义（本判定全程由 wh-review 在本地执行，3rd-review 不参与、不感知）**：
- 适用范围：`total_round ≥ 2`（异源 round2/round3 与同源模式各轮均适用）。
- **判定基线（round23 修复：历史全集而非仅上一轮；round28 修复：历史全集须含被本 FR 默认降级为 minor 的条目）**：某条 finding 是否构成"round2+ 新发现"，须检查其 `finding_fingerprint`（定义见 FR-WHREVIEW-003）是否在同一 `review_flow_id` 的 `round-state-{stage}-{review_flow_id}.json` 的 `finding_fingerprints` 数组（覆盖该审查流程全部历史轮次，含 `severity_decision=default_downgraded_to_minor` 的历史条目，定义见 FR-WHREVIEW-003"字段完整性、历史记录与自校验"/data-contracts.md Contract 4，非仅上一轮报告）中出现过——若遗漏查找 `severity_decision` 为降级为 minor 的历史条目，会把此前已被本规则默认降级过的 finding 每轮都当成"从未出现过"的真正新发现重复降级，永远不会因"曾出现过"而维持原判定，round28 finding：
  - **从未出现过** → 判定为"**真正新发现**"，按下方默认规则处理。
  - **曾出现过**（不论对应条目 `last_status` 当前是 `resolved`〈已闭合后重新出现〉还是 `open`〈含此前某轮出现、后续轮次报告暂时未再列出但从未被明确闭合的情形〉）→ 统一判定为"**重新开放的历史发现**"，不适用本 FR 的降级规则，直接按 3rd-review 本轮对该 finding 判定的原始严重度对待（blocking 维持 blocking，不因"round2+ 新发现"而降级为 minor）。此前版本仅比对上一轮三类列表，会把"早先出现过、上一轮暂时消失、本轮再次出现"的历史发现误判为新发现、默认降级放过真实回归（round23 finding，予以订正）。**round26 修复：判定"是否算重新开放"不再要求存在 `root_cause_diagnoses` 记录作为前提**——此前版本要求"`resolved` 后以不同根因重新出现"须比对 `root_cause_diagnoses` 记录才能判定为"真正新发现"，但很多 finding 在 `last_status` 转为 `resolved` 之前从未达到 FR-WHREVIEW-003 定义的"连续2轮未闭合"阈值，根本没有触发过根因诊断步骤，也就没有 `root_cause_diagnoses` 记录可比对，导致这类 finding 重新出现时无法可靠区分"新发现"还是"重开的历史发现"（round26 finding）。本轮起，只要 `finding_fingerprints` 历史全集中存在该指纹（不论其 `last_status` 是 `resolved` 还是 `open`），即默认判定为"重新开放的历史发现"，这是保守的默认行为——避免把回归问题误判成"真正新发现"从而被默认降级规则错误放过。`root_cause_diagnoses` 记录（若确实存在）仅作为**额外的加强判据**，用于报告标注更细粒度的场景（如区分"同根因复发"与"不同根因但同一定位点"），不是判定"是否算重新开放"的必要前提，缺失该记录不影响本条判定结论。
- 默认规则（仅适用于上方判定为"真正新发现"的 finding）：本轮被判为 blocking 的"真正新发现"，则**默认降级为 minor**：不计入本轮 `blocking_count`，不阻断本轮裁决为 `pass`。
- 例外（仅适用于"真正新发现"路径；满足以下三类之一，仍算 blocking，不降级）：
  (a) **本轮改动新引入的问题**：该 finding 的定位点落在本轮 diff / Delta Package 实际改动的文件范围内（上一轮之后才新增或修改的内容引入的问题）；判定须比对 FR-WHREVIEW-006 持久化的"上一轮 materials 基线"（文档类为快照 diff、代码/测试/决策类为 data-contracts.md Contract 12 定义的 git 基线记录），确认该定位点确实落在相对上一轮基线新增/修改的范围内，不得凭主观印象判定。
  (b) **上一轮审查范围内客观无法发现的问题**：该 finding 所依赖的上下文/证据在上一轮 materials 中确实不存在或不可见（如依赖本轮才补充的外部信息或运行时行为），非"上一轮遗漏"；判定须比对 FR-WHREVIEW-006 持久化的"上一轮 materials 基线"（同上，文档类/代码类分别对应快照 diff / Contract 12 基线记录），确认该上下文在上一轮基线中确实不可见，不得凭主观印象判定。
  (c) **触碰架构边界 / scope boundary 违规**：该 finding 属于 spec.md §9 Scope Boundary 或架构边界定义范畴内的越界问题，严重度不因"round2+ 新发现"而降级。
- **round24 修复：判定主体改为 wh-review**——wh-review 完成本地判定后，须为每条命中本 FR 判定逻辑的 finding 计算 `severity_decision`（枚举定义见 data-contracts.md Contract 4），并同步写入自身维护的 `round-state-{stage}-{review_flow_id}.json` 对应 `finding_fingerprints` 条目与其渲染的最终报告 Blocking Issues/Minor Issues 对应条目，显式标注判定依据：`真正新发现-降级为 minor（默认规则）`、`真正新发现-维持 blocking（命中例外 a/b/c，注明具体理由）`，或 `重新开放的历史发现-维持原严重度（不适用降级规则）`，供人工核查，不得只给出裁决结果而不给依据；3rd-review 返回的原始 findings 不要求、也不产出 `severity_decision` 字段（3rd-review 对该判定保持零知识）。

**Given/When/Then**：
- Given `total_round ≥ 2`，本轮产出一条 finding，其 `finding_fingerprint` 在同一 `review_flow_id` 的历史 `finding_fingerprints` 全集中从未出现过，且判定为 blocking；
- When 该 finding 不满足例外 (a)(b)(c) 任一；
- Then 该 finding 须重分类为 minor，不计入 `blocking_count`，不阻断本轮裁决为 `pass`（其余 blocking findings 情况仍独立正常判定）。
- Given `total_round ≥ 2`，本轮产出一条 finding，其 `finding_fingerprint` 曾出现在同一 `review_flow_id` 的历史 `finding_fingerprints` 全集中（不论对应条目 `last_status` 当前是 `open`〈即中间某轮报告暂时未再列出，但从未被闭合〉还是 `resolved`〈已明确闭合后重新出现，且不要求存在对应的 `root_cause_diagnoses` 记录〉）；
- When 本轮再次出现且判定为 blocking；
- Then 该 finding 判定为"重新开放的历史发现"，不适用降级规则，维持 blocking，计入本轮 `blocking_count`；即便该指纹对应的 `root_cause_diagnoses` 记录不存在（该 finding 此前从未触发过连续2轮未闭合的诊断阈值），本判定结论不变。

**验收标准**：
- AC-DOWNGRADE-1（本地实现落地性，round24 修复：判定主体由"3rd-review 依据合同判定"改为"wh-review 本地计算"，不再要求写入 3rd-review 调用所依据的 stage 专属合同）：wh-review 侧实现该规则的脚本（`skills/wh-review/scripts/round-state.mjs`，见 tasks.md T011）须包含本规则的判定基线（历史全集而非仅上一轮）、默认降级判据、(a)(b)(c) 三类例外定义，以及"重新开放的历史发现"处理规则，并有对应单元测试覆盖（`round-state.test.mjs`）；5 套 stage 专属合同文本（FR-WHREVIEW-002）不要求包含本规则内容——3rd-review 对 stage/round 历史保持零知识（见 FR-THIRDREVIEW-001），物理上无法执行该判定，写入合同文本对其裁决行为不产生实际效果。
- AC-DOWNGRADE-2（行为验证，默认降级；round23 修复：判据改为历史全集）：给定模拟 round2 场景——某 finding_fingerprint 在同一 review_flow_id 的历史 `finding_fingerprints` 全集中从未出现过、本轮判 blocking、不落在本轮 diff 改动范围内、非"客观无法发现"、不触碰 scope boundary——断言最终归类为 minor，不计入 `blocking_count`。
...
- AC-DOWNGRADE-5（行为验证，重新开放的历史发现，round23 新增）：给定某 finding_fingerprint 在 round1 出现并判定为 blocking（`last_status` 从未被标记为 `resolved`），round2 报告中未再列出该 finding（暂时消失一轮），round3 该 finding_fingerprint 又重新出现且判定为 blocking——断言该 finding 判定为"重新开放的历史发现"，不适用降级规则，仍计入 round3 的 `blocking_count`，不因"round2+ 新发现"误判而降级为 minor。
- AC-DOWNGRADE-6（行为验证，重新开放的历史发现——`resolved` 且无诊断记录场景，round26 新增）：给定某 finding_fingerprint 在 round1 出现并判定为 blocking，round2 该 finding 已修复、`last_status` 被标记为 `resolved`（且该 finding 此前从未达到 FR-WHREVIEW-003"连续2轮未闭合"阈值，从未触发过根因诊断步骤，`root_cause_diagnoses` 中无对应记录），round3 该 finding_fingerprint 又重新出现且判定为 blocking——断言该 finding 判定为"重新开放的历史发现"，不适用降级规则，仍计入 round3 的 `blocking_count`，不因"`root_cause_diagnoses` 无对应记录"或"round2+ 新发现"误判而降级为 minor或漏判。

---

### FR-WHREVIEW-006 上一轮 materials 基线持久化机制（通用；文档类走快照 diff，代码/测试/决策类走 git 基线记录，round25 修复：由"仅文档类"扩展为全 stage 通用机制）

**描述**：spec.md/data-contracts.md/plan.md/tasks.md 等文档类审查对象，round2+ 不再拼接文档全文送审，改为传"文档快照 diff"——round(N-1) 快照与 round N 当前内容做文本 diff，作为本轮 Delta Package materials 的构成部分（与 FR-WHREVIEW-003 既有"第2轮起增量降级构造 Delta Package"条款配合）。**round23 修复：materials 归属单一权威来源**——wh-review 是文档类审查对象 snapshot diff 生成与最终 `materials` 组装的唯一 owner：本轮送审 `materials` 字段内容（含 round2+ 的文档快照 diff）由 wh-review 自身基于本 FR 定义的快照机制直接生成/组装完成，不依赖读取 FR-WHREVIEW-007 定义的 `prompt-{review_flow_id}-r{N}.md` 文件内容作为 `materials` 本体来源；该文件在 FR-WHREVIEW-007 中的定位收紧为补充说明/上下文摘要，不与本 FR 的快照 diff 竞争权威（此前版本 FR-WHREVIEW-007 要求 wh-review 直接把该文件内容当 `materials`，与本条形成双重真相源，round23 finding，予以订正）。

**round25 修复：本机制由"仅文档类"扩展为全 stage 通用机制**——FR-WHREVIEW-005 round2+ 新发现降级规则的例外 (a)（本轮新引入的改动）与例外 (b)（上一轮客观发现不了）都需要拿"上一轮审查所用 materials 的基线"做比对，此前版本仅本 FR 为文档类审查对象定义了快照持久化，build-code/verify-code/make-decision 等 stage 的代码/测试/决策类审查对象没有对应的"上一轮 materials 基线"持久化机制，导致例外 (a)(b) 在这些场景下无判定依据、无法落地（round25 finding）。本轮起，"每轮审查完成后持久化该轮实际使用的 materials 基线，供下一轮判定例外 (a)(b) 时读取比对"是**全部 stage 的通用要求**，按审查对象类型分两种落地方式，两者须同时满足"能支撑例外 (a)/(b) 判定"这一共同目标：
- **文档类审查对象**（spec.md/data-contracts.md/plan.md/tasks.md）：沿用本 FR 原有的快照 diff 机制（见下方"快照存放路径"/"生成时机"条款），落盘当轮文档完整内容快照，供下一轮做文本 diff 判定"本轮改动范围"（支撑例外 a）与"上一轮可见范围"（支撑例外 b）。
- **代码/测试/决策类审查对象**（build-code/verify-code/make-decision 等 stage 送审内容为源码 diff、测试报告、决策合同等非文档类内容）：改用 git 基线记录 + materials 全文快照两部分组成的持久化方式——每轮审查提交（调用 3rd-review 引擎）之前，wh-review 落盘一份基线记录（结构与路径见 data-contracts.md Contract 12），须同时包含：①元数据字段 `git_sha`（或等效 commit hash/版本锚点）、本轮 materials 内容 hash `materials_content_hash`、`covered_paths`；②本轮实际提交给 3rd-review 的完整 materials 内容全文（或等效 diff/patch 文本），落盘至独立快照文件，存放路径参考文档类快照的命名风格（见 data-contracts.md Contract 12）。**round26 修复：仅存 hash 摘要不足以支撑"可见性"判定**——FR-WHREVIEW-005 例外 (b) 要求判断"该 finding 依赖的上下文在上一轮 materials 中是否真的可见"，光有 `materials_content_hash` 只能证明内容是否发生变化，无法比对具体内容是否包含某段上下文；必须能读取上一轮实际提交的 materials 全文（或等效 diff/patch）才能完成该判定（round26 finding）。因此②的全文/diff 持久化是每轮的强制要求，不再是"具体是存全量内容还是仅存 hash 由实现自行选择"的可选项；`git_sha`/`covered_paths` 仅作为辅助溯源信息使用，不作为判定可见性的唯一依据。

两种落地方式共同遵守：①每轮调用 3rd-review 引擎之前必须完成当轮基线落盘；②基线记录一经生成不得覆盖或删除；③文件名/记录键须含 `{review_flow_id}`，避免同一 stage 先后发起的不同审查流程互相覆盖（原因同下方 round14 finding）；④下一轮判定 FR-WHREVIEW-005 例外 (a)(b) 时读取上一轮基线记录失败或缺失，须 fail-loud 报错，不得静默跳过例外判定、直接按默认降级规则处理。

**快照存放路径（文档类）**：`tasks/{task-id}/reviews/snapshots/{doc}-{review_flow_id}-r{N}.md`；`{doc}` 取被审文档的基础文件名（不含目录与扩展名，如 `spec`/`plan`/`tasks`/`data-contracts`）；`{review_flow_id}` 为本次审查流程的稳定唯一 ID（定义与生成规则见 FR-WHREVIEW-003"字段完整性、历史记录与自校验"）；`{N}` 为该文档本次参与审查对应的 `total_round`。代码/测试/决策类审查对象的基线记录路径见 data-contracts.md Contract 12。

**生成时机**：每轮审查提交（即调用 3rd-review 引擎）之前，wh-review 自动生成当轮基线（文档类为完整内容快照，代码/测试/决策类为 Contract 12 定义的 git 基线记录），不依赖人工手工触发；基线一经生成不得覆盖或删除。**注意**：`total_round` 会在同一 stage 每次发起新审查流程时从 1 重新计数，仅靠 `{doc}-r{N}.md`（不含 `{review_flow_id}`）不足以保证各轮文件名天然不冲突——同一 stage 先后两次审查流程都可能各自产出 `{doc}-r1.md`（round14 finding，订正此前"各轮独立文件名，天然不冲突"的错误断言）；文件名纳入 `{review_flow_id}` 后才能保证不同审查流程之间不会覆盖彼此的历史快照。
...
- Given 本轮待审查对象属于代码/测试/决策类（build-code/verify-code/make-decision 等 stage 送审内容为源码 diff/测试报告/决策合同）且 `total_round ≥ 2`；
- When wh-review 依据 FR-WHREVIEW-005 判定本轮某 blocking finding 是否命中例外 (a)(b)；
- Then wh-review 须读取上一轮持久化的基线记录（Contract 12，含元数据 JSON 与 materials 全文快照两部分）比对该 finding 定位点是否落在本轮改动范围内（例外 a）、其依赖的上下文是否在上一轮 materials 全文中真的可见（例外 b，round26 修复：直接比对全文而非仅比对 hash）；上一轮基线记录（含全文快照）缺失时 fail-loud 报错，不得静默跳过例外判定。

**验收标准**：
- AC-SNAPSHOT-1：`total_round = 1` 时不生成 diff，materials 仍为全文（沿用现状）；调用 3rd-review 前必须落盘当轮快照 `{doc}-{review_flow_id}-r1.md`。
- AC-SNAPSHOT-2（行为验证；round23 修复：明确 materials 由 wh-review 自身直接生成，不经由 FR-WHREVIEW-007 定义的 prompt 文件中转）：`total_round ≥ 2` 时，断言 materials 内容为 round(N-1) 快照 vs 当前文档的文本 diff（非全文，且非取自 `prompt-{review_flow_id}-r{N}.md` 文件内容），且调用前已落盘当轮快照 `{doc}-{review_flow_id}-r{N}.md`。
...
- AC-SNAPSHOT-5（非文档类基线持久化，round25 新增；round26 修复：新增 materials 全文快照文件的强制持久化验证）：给定 build-code/verify-code/make-decision 等非文档类审查对象完成一轮审查，断言调用 3rd-review 引擎前已落盘 Contract 12 定义的基线记录（含非空 `git_sha`/`materials_content_hash`/`covered_paths` 三字段的 JSON 元数据文件，以及持久化本轮实际提交 materials 全文的快照文件，路径与命名见 data-contracts.md Contract 12）；两部分文件均不得缺失——仅落盘 hash 摘要不足以支撑例外 (a)/(b) 的可见性判定，materials 全文快照为强制项而非可选项（round26 finding）；给定下一轮判定 FR-WHREVIEW-005 例外 (a)(b) 时该基线记录（含全文快照）缺失，断言 wh-review 报错退出，不静默跳过例外判定。

---

### FR-WHREVIEW-007 审查提示词文件生成子代理机制

**描述**：各 stage 主 agent（make-decision/build-spec/build-plan/build-code/verify-code 五者之一，stage 执行 agent 本身）在调用 wh-review 执行实际审查前，须派生一个子代理，基于当前 materials/diff（含 FR-WHREVIEW-006 文档快照 diff）与最少必要上下文（如 stage 名称等元信息），生成审查提示词补充说明文件并写入磁盘；wh-review 读取该文件内容作为组装 `materials` 时的**补充上下文**，而非由 stage 主 agent 自己在其主上下文中拼装提示词/materials。**round23 修复：materials 归属单一权威来源**——`materials` 字段本体（文档类审查对象为 round2+ 快照 diff，见 FR-WHREVIEW-006）由 wh-review 自身直接生成/组装完成，不采信该补充说明文件内容作为 `materials` 本体来源，该文件不与 wh-review 自身生成的 diff 竞争权威。目的：减少 stage 主 agent 自身的上下文消耗，避免其主上下文直接持有大段 materials 全文，同时保持 materials 组装单一权威来源为 wh-review 自身。**round22 修复：合同单一权威来源**——该提示词文件禁止内嵌合同全文，子代理不读取、也不拼装 `contract_path`（见 FR-WHREVIEW-002）指向的合同内容；审查 agent/3rd-review 引擎需要合同原文时，统一由 wh-review 独立读取 route-decision-{stage}-{review_flow_id}.json 记录的 `contract_path`/`contract_hash` 赋值给三元组的 `contract` 字段单独传输，与本文件在物理层面完全分离，消除同一份合同"prompt 内嵌版本"与"route-decision 记录版本"不一致时读到哪份不确定的风险（round22 finding，详见下方"该文件与 Contract 1 三元组的关系"段）。
...
- **步骤1（准备）**：stage 主 agent 先以"准备模式"调用 wh-review（复用 wh-review 现有入口，附加准备语义，不新增独立可执行文件）。**round27 修复：D2 人工确认门前置检查，堵住"复用既有流程后仍放行下一轮"绕过人工确认门的缺口**——wh-review 在执行下方①②③之前，必须先按 data-contracts.md Contract 4 附属"活跃审查流程指针文件"定义的 prepare 判断顺序，读取 `active-flow-{stage}.json` 定位既有 `round-state-{stage}-{既有review_flow_id}.json`；若该文件命中的具体子状态是 `verdict=pass` 且 `post_review_action=await_human_confirmation` 且对应人工确认 artifact `human-confirmation-{stage}-{既有review_flow_id}-{total_round}.json` 尚不存在或字段不匹配（即 FR-D2-001 定义的人工确认门仍悬而未决），wh-review 必须直接返回明确的 `blocked_by_human_confirmation` 状态给调用方（返回值 schema 定义见下方"输出契约"），且本次调用不得执行以下任何一项：不分配/不计算下一个 `total_round`（下方①②均不执行）；不返回可供步骤2子代理使用的三元组（阻止步骤2生成新的 prompt 文件）；不允许调用方进入步骤3执行审查。调用方收到 `blocked_by_human_confirmation` 后必须停在原地等待人工确认，不得重试推进；只有对应人工确认 artifact 生成后，下一次准备调用才恢复走下方①②③正常流程。此前置检查只在该特定子状态命中时短路整个准备流程；其余"进行中"场景（`verdict` 尚未到达终态、或 `verdict=revise_required` 需继续下一轮）不受影响，仍按下方①②③正常推进并复用既有 `review_flow_id`。wh-review 在此步完成三件事并返回给调用方：①分配或复用 `review_flow_id`（同一进行中审查流程复用既有值，全新流程生成新值，规则见 FR-WHREVIEW-003"字段完整性、历史记录与自校验"；round22 修复：分配或复用后须同步写入/更新 `tasks/{task-id}/reviews/active-flow-{stage}.json` 指针文件，字段与写入规则见 data-contracts.md Contract 4 附属"活跃审查流程指针文件"）；②计算下一个 `total_round`（读取当前 `round-state-{stage}-{review_flow_id}.json` 当前值 +1，流程首次调用为1；复用既有流程时按 data-contracts.md Contract 4 附属"活跃审查流程指针文件"定义的发现规则——读取 `tasks/{task-id}/reviews/active-flow-{stage}.json` 指针文件取得 `review_flow_id`，据此拼出该文件路径——找到该文件；round22 修复：不再以 mtime 通配排序作为默认发现路径，mtime 仅作指针文件丢失/损坏时的人工兜底排障手段）；③解析 `contract_path`（**直接复用 FR-WHREVIEW-002 已定义的 stage→合同映射路由逻辑，并完成 route-decision-{stage}-{review_flow_id}.json 的第一次写入——此时只写入 `stage`/`contract_path`/`contract_hash`/`timestamp`/`input_mode`/`review_flow_id`/`total_round`，`review_input_hash` 留空占位，两阶段写入规则见 FR-WHREVIEW-002"route-decision 记录文件契约"，见 Contract 3；FR-WHREVIEW-007 不重新实现 FR-WHREVIEW-002 的路由逻辑，只消费其解析结果**）。这三个值可写入临时 `tasks/{task-id}/reviews/round-prep.json` 或作为调用返回值，具体落盘形式由实现选择，但调用方必须能确定性获得三者。
- **步骤2（生成 prompt，round23 修复：定位收紧为补充说明，不再是 materials 来源；round27 修复：拼出落盘路径前须先执行 task_id 路径安全校验，堵住"子代理先行写盘、wh-review 事后才校验"的时序缺口）**：stage 主 agent 拿到步骤1返回的 `review_flow_id`/`total_round` 后，派生审查提示词生成子代理，子代理**必须先对 `task_id` 执行 FR-WHREVIEW-001"路径安全约束"定义的同一套 `^[A-Za-z0-9._-]+$` 字符集校验**——校验不通过须 fail-loud（非零退出+明确错误信息）拒绝生成/写入该文件，不得静默清洗或截断后继续拼接路径，也不得先落盘写入再依赖 wh-review 下一次调用（步骤3）才发现问题；校验通过后，子代理使用这两个已知值（而非自行猜测或重新计算）拼出落盘路径，读取当前 materials/diff 与最少必要上下文（stage 名称等元信息），生成审查提示词补充说明文件并写入磁盘——该文件只承载补充说明/上下文摘要，不构成最终 `materials` 本体；`contract_path` 仅供 wh-review 步骤1路由解析与后续 `contract` 字段独立赋值使用，子代理不读取、不消费该字段，提示词文件中不得出现合同原文（round22 修复）。
- **步骤3（执行审查）**：stage 主 agent 再调用 wh-review 执行实际审查，把步骤2产出的 prompt 文件路径传入；wh-review 读取该文件作为组装 `materials` 的补充上下文，`materials` 字段本体由 wh-review 自身直接生成——文档类审查对象复用 FR-WHREVIEW-006 快照 diff 机制作为 materials 核心内容（round23 修复：消除与 FR-WHREVIEW-006 对 materials 归属的双重权威冲突），并在真正调用 3rd-review 引擎之前基于该实际 `materials` 计算 `review_input_hash`，完成 route-decision-{stage}-{review_flow_id}.json 的第二次写入（就地回填该字段，见 FR-WHREVIEW-002"route-decision 记录文件契约"）；`contract` 字段仍以步骤1中 wh-review 自己解析并已写入 route-decision-{stage}-{review_flow_id}.json 的 `contract_path`/`contract_hash` 为准，不信任、也不重新解析子代理传入或自行推断的 contract 路径，避免 contract 解析出现双重权威。

**触发者**：各 stage 主 agent，步骤1/步骤3触发时机均为该 stage 收尾、准备调用 wh-review 前后；步骤2（派生子代理）发生在步骤1返回之后、步骤3之前。

**输入契约**：步骤1（准备调用）输入 `stage`；步骤2（提示词生成子代理）接收步骤1返回的 `review_flow_id`/`total_round`，以及当前 materials/diff；`contract_path` 不下发给该子代理（round22 修复）。

**输出契约（round28 修复：步骤1"三元组"表述改为带 `status` 字段的两分支返回契约，堵住调用方无法据此实现稳定分支判断代码的缺口）**：步骤1（准备调用）的返回值须带显式 `status` 字段，区分两种场景，字段集互不叠加——
  - **正常场景（`status="ready"`）**：返回 `{status: "ready", review_flow_id, total_round, contract_path}` 四字段均非空；`contract_path` 仅供 wh-review 自身后续赋值 `contract` 字段使用，不透传给步骤2子代理。
  - **被人工确认门阻塞场景（`status="blocked_by_human_confirmation"`，对应上方"人工确认门悬而未决"分支）**：返回 `{status: "blocked_by_human_confirmation", review_flow_id}` 两字段，不含 `total_round`/`contract_path` 键（键不存在或为空均视为符合）——这两个值在该场景下未被计算，不得返回猜测值或复用上一轮遗留值。

调用方只在收到 `status="ready"` 时才派生步骤2提示词生成子代理；收到 `status="blocked_by_human_confirmation"` 时必须停在原地等待人工确认，不得派生子代理、不得进入步骤3执行审查。步骤2子代理将本轮送审的补充说明/上下文摘要（不含合同原文，不含 `materials` 本体，round23 修复）写入 `tasks/{task-id}/reviews/prompt-{review_flow_id}-r{N}.md`（`{review_flow_id}`/`{N}` 均取步骤1 `status="ready"` 返回值，非子代理自行生成或猜测），并把该路径返回给 stage 主 agent；stage 主 agent 自身上下文不直接持有 materials 全文，仅持有该文件路径；`materials` 字段本体由 wh-review 自身在步骤3生成/组装。

**该文件与 Contract 1 三元组的关系（round23 修复：收紧为补充上下文定位，消除与 FR-WHREVIEW-006 的 materials 归属双重权威冲突；round22 修复：收紧为单一合同权威来源，消除双重权威风险；此前 round14 finding 修复的表面冲突结论予以订正）**：`prompt-{review_flow_id}-r{N}.md` 只承载本轮送审的补充说明/上下文摘要（如 stage 名称等元信息，不含 materials 本体），**禁止内嵌合同全文**——它不是 `{mode, contract, materials}` 三元组中 `materials` 字段的来源，与 `contract` 字段的产生同样完全无关（round23 修复：此前版本表述"它只是 materials 字段的来源"与 FR-WHREVIEW-006"wh-review 自身生成 snapshot diff 作为 materials"存在双重真相源冲突，予以订正）。wh-review 调用 3rd-review 引擎组装该三元组时：
- `mode` 由 wh-review 自身根据当轮 `round-state-{stage}-{review_flow_id}.json` 判定值独立赋值（`full`/`incremental`/`same-source`），不依赖对该文件的解析获得；
- `contract` 由 wh-review 独立读取 `route-decision-{stage}-{review_flow_id}.json` 记录的 `contract_path`/`contract_hash`（见 Contract 3）赋值，物理上与本文件分离，不依赖对该文件的解析获得；
- `materials` 字段本体由 wh-review 自身生成/组装（文档类审查对象复用 FR-WHREVIEW-006 已定义的快照 diff 机制，作为 materials 的核心部分）；wh-review 可将本文件内容作为补充上下文纳入最终 materials 的辅助说明部分，但本文件内容不构成 materials 本体、不与 wh-review 自身生成的 diff 竞争权威。

此前版本允许该文件同时内嵌合同原文与 materials（理由是"便于审查方阅读理解上下文，不违反 Contract 1 的独立字段校验"），但这会产生一份游离于 route-decision 记录之外的"第二份合同文本"：当 prompt 内嵌的合同与 `contract_path`/`contract_hash` 指向的合同不一致时（如落盘时序错位、合同文件被并发修改），审查 agent 实际读到哪份没有强约束，可能导致审查结果漂移（round22 finding）。本轮改为物理分离方案：审查 agent/3rd-review 引擎需要阅读合同原文时，统一通过 `contract_path` 单独读取，不依赖本文件，从根源上消除两份合同不一致的可能。

三者仍作为三个独立字段整体序列化传给 3rd-review 引擎（见 Contract 1"传输方式"），`prompt-{review_flow_id}-r{N}.md` 只负责提供 `materials` 的补充上下文部分，`materials` 本体由 wh-review 自身生成（round23 修复）。

**Given/When/Then**：
- Given 某 stage 主 agent 已完成本 stage 工作，准备发起审查，且当前无悬而未决的人工确认门；
- When 该 stage 主 agent 先以准备模式调用 wh-review；
- Then wh-review 在准备调用中完成 `review_flow_id` 分配/复用、`total_round` 计算、`contract_path` 解析（复用 FR-WHREVIEW-002 路由逻辑）并返回 `{status: "ready", review_flow_id, total_round, contract_path}`；stage 主 agent 拿到该返回值后派生审查提示词生成子代理，子代理只用 `review_flow_id`/`total_round` 两个已知值（而非自行计算）拼出落盘路径，读取当前 materials/diff 与最少必要上下文（不读取、不内嵌 `contract_path` 指向的合同原文），写出补充说明文件 `tasks/{task-id}/reviews/prompt-{review_flow_id}-r{total_round}.md`（不含 `materials` 本体，round23 修复）；stage 主 agent 自身进程内不直接拼接 materials 全文；stage 主 agent 再调用 wh-review 执行实际审查并传入该 prompt 文件路径，wh-review 读取该路径文件内容作为**补充上下文**，`materials` 字段本体由 wh-review 自身直接生成（文档类审查对象复用 FR-WHREVIEW-006 快照 diff 机制），`mode`/`contract` 两个独立字段仍分别来自 round-state-{stage}-{review_flow_id}.json/准备调用已写入的 route-decision-{stage}-{review_flow_id}.json，三者一并完成后续调用。
- Given 某 stage 主 agent 已完成本 stage 工作，准备发起审查，但既有审查流程命中"人工确认门悬而未决"子状态（`verdict=pass` 且 `post_review_action=await_human_confirmation` 且对应人工确认 artifact 尚不存在或字段不匹配）；
- When 该 stage 主 agent 以准备模式调用 wh-review；
- Then wh-review 返回 `{status: "blocked_by_human_confirmation", review_flow_id}`，不含 `total_round`/`contract_path`；stage 主 agent 停在原地等待人工确认，不得派生提示词生成子代理，不得进入步骤3执行审查。
...
- AC-PROMPT-3（fail-loud；round23 修复：该文件职责由 materials 来源调整为补充上下文，措辞同步更新，fail-loud 语义不变）：wh-review 读取 `prompt-{review_flow_id}-r{N}.md` 失败或文件不存在时报错退出，不静默回退为空补充上下文。
- AC-PROMPT-4（三元组字段独立性，round14 新增；round22 修复：措辞对齐"禁止内嵌合同全文"新规则）：断言 wh-review 传给 3rd-review 引擎的 `mode`/`contract` 字段值分别可追溯至 `round-state-{stage}-{review_flow_id}.json`/`route-decision-{stage}-{review_flow_id}.json`，不依赖对 `prompt-{review_flow_id}-r{N}.md` 文本内容的解析提取；即便该文件完全不含合同原文（round22 起强制要求），`mode`/`contract` 字段值仍不受影响。
- AC-PROMPT-7（合同单一权威来源，round22 新增）：断言 `prompt-{review_flow_id}-r{N}.md` 文件内容不包含 `contract_path` 指向的合同文件的原文（可用字符串包含检测：合同文件全文内容不作为子串出现在 prompt 文件中）；断言提示词生成子代理的执行轨迹中不存在读取 `contract_path` 指向文件的调用记录。
- AC-PROMPT-5（两段式调用顺序 + 阻塞场景返回契约，round16 新增修复循环依赖；round28 修复：新增对 `status` 字段两分支返回值的验证）：①正常场景下，断言调用链中存在一次先于提示词生成子代理派生的"准备"调用，该调用返回 `{status: "ready", review_flow_id, total_round, contract_path}` 四字段均非空，其中 `review_flow_id`/`total_round` 与最终落盘的 `prompt-{review_flow_id}-r{N}.md` 文件名中的值完全一致（即子代理消费的是准备调用返回值，而非自行生成或猜测）；②断言步骤3实际执行审查时使用的 `contract_path`/`contract_hash` 可追溯至准备调用阶段写入的 route-decision-{stage}-{review_flow_id}.json，与提示词生成子代理无关，验证 contract 解析权威唯一；③给定命中"人工确认门悬而未决"子状态的模拟场景，断言准备调用返回值为 `{status: "blocked_by_human_confirmation", review_flow_id}`，不含 `total_round`/`contract_path` 两键（键不存在或为空均视为符合），且调用链中不出现提示词生成子代理派生记录、不出现步骤3执行审查的调用记录。
- AC-PROMPT-6（route-decision-{stage}-{review_flow_id}.json 两阶段写入，round17 新增；round28 修复：明确本断言仅适用于准备调用返回 `status="ready"` 的场景，并补充阻塞场景的断言）：给定准备调用返回 `status="ready"`，断言步骤1准备调用结束后落盘的 route-decision-{stage}-{review_flow_id}.json 中 `stage`/`contract_path`/`contract_hash`/`timestamp`/`input_mode` 已非空，但 `review_input_hash` 为空（空字符串或 `null`）；断言步骤3执行审查、真正调用 3rd-review 引擎之前，同一份 route-decision-{stage}-{review_flow_id}.json 的 `review_input_hash` 已回填为基于实际 `materials` 计算的非空 hash 值，且其余五个字段值未被覆盖改变。给定准备调用返回 `status="blocked_by_human_confirmation"`，断言该次调用不产生 route-decision-{stage}-{review_flow_id}.json 的任何写入（既有文件内容保持调用前状态不变）。
...
**验收标准**：
- AC5-1：3rd-review SKILL.md 不含 stage 名称枚举（make-decision / build-spec 等）。机器校验不依赖对 SKILL.md 文本做"先排除含'不接受/不解析/不需要/不含/零知识/剥离'等说明性词的整行，再对剩余行模糊搜索 stage/round/checkpoint 概念词"式关键词过滤——该方案按整行匹配排除，真实违规代码行只要与说明性词凑巧同行即会被一并放过（见 round7 审查发现）。改为直接对 3rd-review 调用入口（`run-heterologous-review.mjs`/`standalone.sh`）做 CLI 契约集成测试：断言其只接受 `--diff=<file> --output=<file>` 两个参数，传入 `--stage`/`--round`/`--checkpoint` 任一遗留参数后必须触发非零退出+明确报错，不允许被静默忽略后继续执行（"忽略后继续执行"不再是可接受的通过分支）；并对源码中实际的 `yargs`/`process.argv` flag 定义/解析代码行做精确 grep（而非对文档整行做模糊关键词扫描），断言命中的 flag 名集合恰为 `{diff, output}`。
...
- AC5-5（行为验证，本轮新增，锁定 round30 卡死根因的硬性回归约束）：给定 wh-review 对 3rd-review runner 的单次调用超过配置的超时上限仍未返回（含 runner 内部因调度真实 provider CLI 而挂起的场景），断言 wh-review 必须终止该子进程、将本轮裁决判定为 `escalate_to_human`、`failure_reason` 记为 `timeout`，并以退出码 0 fail-loud 完成——不得无限等待、不得判为其他裁决、不得静默重试——可通过集成测试模拟一个"调用后挂起不退出"的 stub runner 验证（见 plan.md/tasks.md `invoke-review-engine.test.mjs` 超时场景断言）。3rd-review 引擎内部对具体 provider CLI（如 codex/gemini/grok/cursor/claude）的调用方式、进程调度与是否自带超时，属于独立兄弟仓库 `$THIRD_REVIEW_REPO_ROOT` 的内部实现细节，不在本 spec 的显式契约范围内（隐性必达 1：3rd-review 精简后仍须能独立调用）；wh-review 侧本条超时兜底是本 spec 唯一显式约束的边界，不假设、不依赖引擎内部实现是否已处理超时。
