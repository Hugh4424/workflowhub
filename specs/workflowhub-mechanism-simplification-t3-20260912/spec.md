...
- **Then**：行指纹 = 该 stage 自己的材料 scope 指纹；写侧不再回退整材料指纹。C5 规定读侧不因材料/snapshot 差异拒绝既有行，落后 provenance 只作为可读披露（若该投影提供），历史坏行被如实登记为不可恢复
...
- [x] **错误态**：SCN-003（写侧错误指纹会失败；读侧 stale provenance 保持可读并如实披露）/ SCN-005 / SCN-008（失败停在那条、如实记录）
...
- [ ] **AC-FIX-001**：写侧两处回退改用该 stage 的 scope 指纹；新增窄回归测试覆盖 make-decision 与 build-spec 行指纹路径并通过；两类历史行分轨处置——受害任务（PaperBuilder）的历史坏行如实登记为不可恢复、字节未改写、不重跑，本任务自己的 stage 行按 decision-log §13.5 作为现场标本：修复后重跑该 stage end 必须可读回，最后一次材料记账在最后一次 run 之前。读侧遵循任务Ⅱ C5：不得因材料/snapshot provenance 差异拒绝或自动使既有行失效；如投影提供 stale provenance，只作披露。
场景：无 stage outcome 的 stage end 写入与读回。
验证：跑新回归测试与既有行相关用例；修复后重跑本任务 stage end 并复算写入行指纹 = 该 stage scope 指纹；另验证落后 provenance 仍可读且不改变状态。
通过：新测试与既有用例全绿、写入行使用正确 scope 指纹、读侧不因 provenance 落后拒绝既有行、受害历史行登记在册且字节未动。
失败：仍回退整集指纹；或无新测试；或恢复 C5 已删除的 stale 拒绝/自动重跑链；或受害历史行被改写/迁移/重跑。
证据：测试记录、读回输出、登记段。
