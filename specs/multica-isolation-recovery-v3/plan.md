# 实施计划：multica-isolation-recovery-v3

**Task ID**: workflowhub/multica-isolation-recovery-v3  
**Date**: 2026-07-17  
**Spec**: spec.md，accepted build-spec content hash e3c89a9fe936fad46d1ae9fc82767004c3434e51fdd659395626f1b565f4bb6a  
**Status**: Draft for build-plan review

## Accepted Upstream

- v3 accepted spec：e3c89a9fe936fad46d1ae9fc82767004c3434e51fdd659395626f1b565f4bb6a。
- v2 accepted plan：19767f1c28e1bde7416d86e7a7d8cc909d90e05df01e92b7bb7521c8d6667ad3。
- v2 accepted tasks：c9a20085741bdf567177761a3979e62489d8ccf15f6f5cc270329159ad8e60de。
- v2 plan/tasks 只作为显式语义上游；不得复制其 task records、checkpoint 或 review verdict。
...
## Evidence Recovery Architecture

### Canonical phase records

- `phase-subject.v1` freezes `phase_id`, task/release identity, baseline and implementation snapshot `ref/hash/tree_oid`, allowed files, and exact upstream phase result.
- `phase-diff-scan.v1` is derived only from the subject tree pair. Its patch, changed files, allowlist result and content hash are deterministic and caller-independent.
- `phase-result.v1` binds subject, diff, independent review, tests and next-phase eligibility. All three records are create-only.
- Public launcher accepts only project, task, phase ID and canonical refs plus stdin or launcher-authorized staging input. Internal writer, task path, cwd, commit, range and caller-built diff never cross the boundary.
- `wh-review --phase-id` resolves the canonical subject and regenerates the full tree diff. No ephemeral commit or commit ancestry is required.

### Three-worktree migration

1. Primary worktree is the immutable baseline/reference source. It is not a replay destination.
2. v2 worktree is a read-only migration source for the Phase 0/P0-A diff and accepted design provenance. It cannot publish v3 records.
3. v3 authenticated Workspace is the sole implementation and publisher destination. Every replayed file must be in the frozen allowed-files list and must produce a new v3 snapshot.
4. After Phase 0/P0-A evidence closes, all future work, reviews and records use v3 only. Primary and v2 become non-authoritative read-only sources; deletion, worktree removal or cleanup requires separate authorization and is not performed by this plan.

## Implementation Phases

### Evidence Bootstrap — phase evidence foundation

1. Add a read-only legacy validator that distinguishes task-attempt.v2, task-accepted.v2 and git-checkpoint-plan.v1 from task-snapshot.v1 without rewriting bytes.
2. Freeze closed schemas for phase subject, diff scan and result; add exact producer/consumer/version registrations and malicious fixtures.
3. Implement tree-pair snapshot verification and deterministic diff generation from canonical refs/hashes/tree OIDs.
4. Implement create-only publishers, public launcher commands, exact phase lineage and a P0-B structural gate.
5. Convert wh-review to phase-ID-only resolution and bind every review result to the subject/diff/tree pair.
6. Publish and independently review the canonical Phase 0 evidence before replay begins.

完成条件：legacy fixture bytes/hash unchanged；phase subject/diff/result 可重算且 create-only；所有 caller-selected path/commit/range/diff 在写入前拒绝；Phase 0 canonical review 已闭合。
Maps to：INV-006/007、FR-044—050、FR-052、AC-022—026/028、NFR-007/008。

### Controlled Replay — recover v2 Phase 0/P0-A

1. Freeze primary/v2/v3 identities, v2 baseline and implementation tree, binary diff hash, changed files and allowed-files list.
2. Apply only the validated v2 Phase 0/P0-A patch into v3 Workspace. Reject missing, extra, renamed or drifted files before publication.
3. Run phase-targeted tests, full `npm test`, `npm run check`, syntax checks and `git diff --check`; record every pass/fail/unknown without quality gating.
4. Publish the v3 P0-A subject/diff/result and run independent wh-review against the new v3 tree pair.
5. Keep P0-B structurally unavailable until both Phase 0 and P0-A lineage/reviews are present and hash-valid.

完成条件：v2 records/verdicts 未复制；replayed changed files 精确等于 allowlist；v3 snapshot/diff 可重算；Phase 0/P0-A canonical reviews 闭合；之后所有工作仅在 v3。
Maps to：FR-045—052、AC-023—028。

### Preserved v2 implementation sequence
