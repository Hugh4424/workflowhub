...
- **Input**：`specs/workflowhub-execution-efficiency-20260902/decision-log.md`（D-001~D-008、C-001~C-003）、`specs/workflowhub-execution-efficiency-20260902/spec.md`（FR-001~012、AC-1~7，SHA-256 `083f253831bf9b47`）
...
- **Deletion boundary**：`tools/host/workflowhub-codex-session-{state,hook,event}.mjs`（T14 原子批内删除，删除条件=身份替代 FR-007 落地且 T9 完成后由本任务接管）。
- **Before**：doctor 只回四字段；writer 不记解析来源；旧树无说明；untracked 哈希捕获过早致整轮重跑；会话绑定族在非 Codex 宿主整体失能；wh-review 0 字节无专属语义。
- **After**：doctor 机器可读一致性段+exit 0；task.json 记 write_resolution_source；旧树根 ARCHIVED.md；捕获点入发布事务；身份=显式>派生、冲突/缺失 fail-closed；0字节→contract_failure、原始流只留 broker/host 私有诊断、任务与公共边界只保留错误码/脱敏消息/摘要哈希、partial 永不当 pass；stage 末总结必列遗漏。
- **Main risk**：与任务 A 在 stage-runtime.mjs 同文件不同区（RISK-P-01）；B5b 可能与 usability 旧改动产生文件冲突（RISK-P-02），由 C-001 直接接管并先做冲突检查。
...
- **Next step**：先完成本次材料重基线并重新确认 build-plan，再进入 build-code，按 P1（并行 RED）→GREEN→P2→P3→P4（T9 后由本任务直接接管）→P5 执行；任何越出 File Boundary 或发现并行冲突的实现立即 STOP。
...
- wh-review broker：review-provider-client.mjs:9、:44-75、:287-288、:316-363（已有 120000ms timeout 与 `PROCESS_TIMEOUT`）；0 字节/非法 JSON 仍是本任务的 `contract_failure` 语义；原始流只在 broker/host 私有诊断中保留，任务材料只记摘要哈希，公共边界只记错误码与脱敏消息；simple-review-runner.mjs:318-319 对外归一 `REVIEW_EXECUTION_TIMEOUT`；比较键复用既有 `material_fingerprint`。
...
六个交付面按文件所有权分五个 Phase：P1 存储一致性（stage-runtime doctor 区+storage-root+writer+canonical-receipt-writer+ops）、P2 B5a（stage-runtime 会话派生区+task-bootstrap 绑定调用区）、P3 C 面（wh-review broker+文档）、P4 B5b 原子批（三件套删除+bridge+五份 SKILL.md，由 C-001 本任务直接接管）、P5 聚合验收。
...
broker 读取 provider 输出处：0字节/非法 JSON→contract_failure 内部标签，原始流只留 broker/host 私有诊断，任务材料只留 stdout/stderr 摘要哈希；四公共边界（CLI stdout JSON/canonical result/报告投影/跨任务 quality facts）只含错误码+脱敏消息+摘要哈希；partial=available-with-failures 永不当 pass；相同 material_fingerprint 不重试；保留 main 已有 120000ms bounded timeout、`PROCESS_TIMEOUT` 和对外 `REVIEW_EXECUTION_TIMEOUT` 映射，不新增第二套 timeout/进程生命周期机制。wh-review SKILL.md 增后台执行+轮询约定段。

### B5b 原子批（FR-006/FR-012）

同一 git 提交内：删三件套、bridge 的 session.session_id 改显式参数（无输入如实 unavailable）、保留 main 已加入的 stage-reflection 内容并在五份 SKILL.md 移除"同一会话自动记录"段、session-event 指令且新增遗漏披露段（stage 末总结必列非 completed step/skill 及原因）、ADR 新建为 0024。T9 完成后按 C-001 由本任务直接接管；启动前检查五份文档、bridge 和三件套没有并行未纳入的修改，之后统一完成整批原子交付。
...
### MODIFY
`tools/cli/stage-runtime.mjs`（doctor 区+会话派生区，T14 继续清除自动绑定消费区）、`runtime/evidence/storage-root.mjs`、`runtime/task/task-handle.mjs`、`tools/cli/task-bootstrap.mjs`、`runtime/stage/stage-context.mjs`、`runtime/evidence/canonical-receipt-writer.mjs`、`runtime/stage/stage-handlers.mjs`（仅 diff 证据捕获点区域）、`skills/catalog.yaml`、`skills/wh-review/skill-bundle.json`、`skills/wh-review/scripts/review-provider-client.mjs`、`skills/wh-review/SKILL.md`、`tools/host/workflowhub-stage-agent-bridge.mjs`（T14 批内）、`tests/integration/vnext-official-stage-run.test.mjs`（T14 批内）、`scripts/__tests__/task-bootstrap.test.mjs`、`tests/contract/material-producer-consumer-roundtrip.test.mjs`、`tests/contract/stage-order-and-host-interaction.test.mjs`、`tests/dsh-transcript.test.mjs`、`tests/left-shift/left-shift-suite.test.mjs`、`tests/stage-interaction-contract.test.mjs`、`workflows/make-decision/SKILL.md`、`workflows/build-spec/SKILL.md`、`workflows/build-plan/SKILL.md`、`workflows/build-code/SKILL.md`、`workflows/verify-code/SKILL.md`（T14 批内）。
...
`tests/contract/session-task-context.test.mjs`、`tests/contract/stage-runtime-session-binding.test.mjs`、`tests/m15-codex-session-hook.test.mjs`（专属旧机制测试随删除退休）。

删除证明：a) 消费方清单完整——F-201/F-202 反向引用确认生产消费者为 stage-runtime、task-bootstrap、stage-agent-bridge 及五份 SKILL.md 文本，全部在本计划 MODIFY 面内（T9 先移除代码消费，T14 同批移除文本消费）；b) 删除条件=身份替代 FR-007 落地且 T8/T9 绿；c) 当前已验证生产/工作流 live consumer 无残留，旧机制专属测试已退休；d) 原子性证明必须由授权后的同一 git 提交确认，未授权前不得宣称完成；e) 决策留痕=ADR 0024 随批创建，避免覆盖 main 的 ADR 0023。
...
- **RISK-P-02**（B5b 与 usability 文件冲突）
- **Affected IDs**：T13、T14、T15
- **Trigger**：T9 完成后发现五份 SKILL.md、bridge 或 session 三件套存在未纳入当前批次的并行修改
- **Consequence**：可能产生半更新状态或覆盖其他任务修改
- **Mitigation or STOP**：本任务按 C-001 直接接管；发现并行冲突就 STOP，先协调边界再继续
- **Handling Stage**：build-plan 排期/build-code 执行
- **Verification**：C-001 接管事实与启动前冲突检查记录于任务卡
...
P1（T1/T3/T5/T6 并行 RED → T2/T4/T7 GREEN）→ P2（T8 RED → T9 GREEN，依赖 T2 同文件分区就绪）→ P3（T10 RED → T11 GREEN，与 P1/P2 全并行；T12 随时）→ P4（T9 完成后本任务接管，T13 RED → T14 GREEN 原子批）→ P5（T15 聚合）。
...
| 全部 | 全部 | 全部 | T15 | 当前快照受影响合约聚焦回归 + 结构/差异检查；全仓回归仅作显式 CI/release 事实 |

## 简洁与工程透镜（步骤 5-6 inline，2026-09-03）

- 简洁透镜：无新机制/无新 npm 依赖/无新公共行为；长任务处理=文本约定（T12）非机制；归档=单文件非迁移系统。通过。
- 工程透镜：RED/GREEN 成对共用 gate_cmd 与 oracle 身份；全局 File Boundary 与 Phase Files 双向闭环；B5b 原子批有明确接管事实+单提交不变量+整体回滚；与任务 A 同文件分区写入 STOP 条件。通过。
...
- F5 责任清晰：B5b 由本任务在 T9 后直接接管，内容验收与并行冲突检查分开 ✓
...
- F9 可证伪不假绿：doctor warnings 可证伪；pending/unavailable/incomplete 事实不伪造通过 ✓
...
- S5 事实如实：unavailable/pending/incomplete 不伪造 ✓
- S6 最小材料：四材料不变，无第五材料 ✓
...
### Files
**MODIFY** `tools/host/workflowhub-stage-agent-bridge.mjs`、`tools/cli/stage-runtime.mjs`（T9 身份替代后的自动绑定消费区）、`tests/integration/vnext-official-stage-run.test.mjs`、`scripts/__tests__/task-bootstrap.test.mjs`、`tests/contract/material-producer-consumer-roundtrip.test.mjs`、`tests/contract/stage-order-and-host-interaction.test.mjs`、`tests/dsh-transcript.test.mjs`、`tests/left-shift/left-shift-suite.test.mjs`、`tests/stage-interaction-contract.test.mjs`（同一消费边界的测试适配）、`workflows/make-decision/SKILL.md`、`workflows/build-spec/SKILL.md`、`workflows/build-plan/SKILL.md`、`workflows/build-code/SKILL.md`、`workflows/verify-code/SKILL.md`；**NEW** `tests/contract/session-binding-removed.test.mjs`、`docs/adr/0024-remove-host-session-binding.md`。
...
### Verify
session-binding-removed.test RED→GREEN；提交内容原子性在获得授权后以单提交事实确认。
### Knowledge
F-201/F-202 引用清单。
### STOP
T9 未完成 → 本 Phase 不启动；T9 完成后本任务直接接管，不等待 usability 合并、取消或任何时间条件。启动前若发现五份 SKILL.md、bridge 或 session 三件套存在其他未纳入本批的并行修改，先 STOP 协调文件边界；不能只做删除。
### Done
AC-6 证据齐（同提交+零残留）。
### Risks and rollback
部分缺失整体 revert；并行冲突先协调边界，不覆盖其他任务修改。
...
T15（聚合卡）
### Verify
启动前先核对 T15 的全部依赖卡均为 `completed`；任一依赖仍为 `pending` 时只记录未启动事实，不执行聚合。依赖满足后，仅通过官方 bounded capture（本卡预算 `120000ms`）执行 T15 明确列出的受影响合约测试，启用 `--bail=1 --no-fileParallelism`，再串行执行结构校验、技能闭包校验和 `git diff --check`；`fullstack` 只表示跨边界风险层级，不等于全仓测试。全仓 `npm run test:safe`/`npm run check` 只在显式 CI/release 任务中执行，并须另有预算与授权，不作为本任务默认验收。
### Knowledge
质量缺失如实。
### STOP
...
### Risks and rollback
B5b 未完成时 AC-6 记 incomplete，而非通过。
