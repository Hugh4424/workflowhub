...
- [ ] **T001 实现纯记录合同与归并算法**  
  ...
  依赖：无。  
  改动范围：仅 `core/fact-indexes.mjs`；复用 `core/canonical-source.mjs` 的 `canonicalJson/contentHash`，不复制 hash 实现。  
...
- [ ] **T003 实现入口预检、可信 transcript registry 与 artifact 投影**  
  ...
  输出：新增 `core/fact-collector.mjs` 的 preflight、严格 entry validator/WeakSet branded registry helper、空 registry missing、登记 JSONL reader 路径和 artifact 固定 ref extractor；新增空的 `config/transcript-sources.mjs`；在 TaskHandle 增加仅枚举 `results/<stage>/attempt-NNNN.json` 的受控 `listStageAttemptRefs(stage)`，覆盖正式但未 accepted 的 attempts；可选来源错误转安全 missing/unknown。  
  依赖：T001、T002。  
  改动范围：`core/fact-collector.mjs`、`config/transcript-sources.mjs`、`core/task-handle.mjs`、`core/__tests__/task-handle.test.mjs`；不得收裸 task/worktree/source path，不做任意目录递归扫描，不扫描 HOME/cwd/私有缓存；生产 registry 为空，只有 launcher-issued 完整登记 capability 可读。  
...
- [ ] **T004 实现 health 与 skills 投影**  
  ...
  输出：`core/fact-collector.mjs` 来源适配 + `core/fact-indexes.mjs` 九域 health 和 M14a-compatible skills inventory。  
  依赖：T001、T003。  
...
- [ ] **T005 实现单任务锁内四文件原子写与结果汇总**  
  ...
  依赖：T003、T004。  
  改动范围：`core/fact-collector.mjs`；锁固定为 `locks/indexes/fact-collection.lock`，写固定为 `indexes/transcript-index.jsonl`、`artifact-index.jsonl`、`flow-health-facts.jsonl`、`skills-inventory.json`；只调用 TaskHandle lock/atomic API。  
...
- [ ] **T007 建立唯一 launcher 与 metrics 接线**  
  ...
  输出：新增 `scripts/collect-task-facts.mjs`；从显式静态登记创建 branded transcript registry（当前为空）；入口 `recordSkeleton`，退出 `finally` 中 `updateOwnResult`，打印结构化采集结果并以 failed 对应非零退出。  
  依赖：T005。  
  改动范围：该 launcher 与 `core/fact-collector.mjs` 的窄入口；禁止 task-path/worktree override、raw config 与 storage search。  
