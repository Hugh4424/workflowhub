...
- [ ] **AC-02**：九个 surface 的缺陷样本只由对应问题顺序审查，并能命中各自严重缺陷。
  ...
  - **通过条件**：有 baseline 配对时严重召回不退步，跨面无关 finding 不作为命中；baseline 未实际执行时，必须有独立 baseline 失败/不可用事实证明不是“配对不足”降级，candidate 的每个严重 mutation 必须在 5 个计划运行位中至少 4 次命中。每个运行位都要有 attempt；失败、超时、不可用仍计入分母且不能算命中，不能删除或缩小分母；该绝对门槛只适用于这个 baseline 缺失分支。
...
- [ ] **AC-20**：每个 candidate surface 都有可回读的质量、稳定性、成本和证据完整性对照事实；不把固定分数当成继续或交付闸门。
  ...
  - **通过条件**：逐面有真实的质量、稳定性、成本和证据完整性事实；有配对时能读出严重召回、误报、执行率、token、时长和 finding 质量的变化；baseline 未实际执行时，只有独立 baseline 失败/不可用事实成立，才使用 AC-02 的 5 个计划运行位绝对门槛，并单独报告执行成本；配对不足不能自动进入该分支；不可计算的部分明确写成 `inconclusive/unavailable`。评测结果只用于决定下一步修复和发布风险，不作为 WorkflowHub 继续、阶段完成或正式 close 的单一许可证。
