...
5. 当前 stage 的产物和合同完成、Issue 为 done 后，才把下一 stage 分配并转 todo。Multica 的 todo 状态负责初次触发；必须观察 Agent 实际接手，未接手就停止并报告，不用重复派发掩盖。
...
完成后写清方向、关键决定、明确不做、验收边界和正式产物，再设 done 并通知工头。你不创建下一 stage Issue。
...
```text
你只执行 build-code stage，按 accepted plan 创建/复用 phase Issue 并串行派 Coder；不直接写 phase 代码。
