...
- **RECOVERY WORKSPACE RED/GREEN**：T016/T017 仅修改 `core/workspace.mjs`、`core/stage-context.mjs`、`scripts/stage-runtime.mjs`、`core/task-kernel-implementation.mjs`、`core/__tests__/workspace-manager.test.mjs`、`scripts/__tests__/stage-runtime-recover-run.test.mjs`、`skills/wh-review/scripts/wh-review-cli.mjs`、`skills/wh-review/scripts/__tests__/wh-review-cli.test.mjs`；`skills/wh-review/skill-bundle.json` 与 `skills/catalog.yaml` 仅同步受影响 bundle hash。kernel implementation 仅用于 runtime-owned previous-run CAS。active make-decision recovery run 的方向/详情审查继承 recovery workspace；普通 run、accepted run 与显式 prepare 保持原严格语义。不得新增 schema、认证 Gate 或 caller 可选路径。
