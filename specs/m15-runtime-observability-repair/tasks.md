...
- **任务状态**：`implementation repair in progress`；既有 build-code 修复已执行，当前仍需真实宿主 outcome 与最终 formal acceptance，不得写成完成。
...
- **plan binding**：`specs/m15-runtime-observability-repair/plan.md` sha256 `89c71b4be43b229951f4978f135bc5e6fe708aec1d8bfa7cd1914a9e183549a9`
- **执行口径**：所有任务都在同一当前 task 内按依赖顺序执行；历史数据只读；review 只记录异源建议，只有 build-code 做严格实现审查。
- **真实宿主口径**：WorkflowHub runtime 不执行 Agent。当前授权的 Codex/WorkflowHub host 必须先真实执行 Stage Agent，再调用 `runtime/stage/stage-agent-outcome-adapter.mjs`；adapter 只经现有 `TaskKernel` 写入并绑定当前 snapshot/materials/manifest/spec-analyze。禁止 fixture、session 扫描、路径猜测和第二套 writer。外部项目不在本任务范围，禁止调用、修改、构建、测试或同步。
...
- **ID**：T013
- **Phase**：Phase 3 — 真实宿主 outcome 与正式验收
- **goal**：定义并验证 `runtime/stage/stage-agent-outcome-adapter.mjs` 的宿主接入边界；只接收已实际执行结果，经现有 `TaskKernel` 认证、绑定和写入。
...
- **source_refs / decision_refs**：D-002、D-003、AC-001、AC-010
- **输入**：当前 task、current snapshot、materials、stage manifest、宿主执行结果。
- **依赖**：T012
- **并行**：否
...
- **task risk**：contract 通过不等于真实宿主已调用。
...
### T014 — 真实宿主产出 current stage outcome

- **ID**：T014
- **Phase**：Phase 3 — 真实宿主 outcome 与正式验收
- **goal**：由真实外部 Stage Agent 执行 current stage，并调用 adapter 产出可供 public `run` 消费的 current stage outcome。
