...
- Task：workflowhub/multica-isolation-recovery
...
- FR-005：所有产品命令必须在认证 Workspace/CandidateWorkspace cwd 执行；CLI 不接受 cwd、worktree、baseline 或跨进程 capability id。
...
- FR-017：CLI 只接受 canonical task/stage/release/source/artifact refs、enum 和 JSON stdin/file payload；不得接受 shell string、任意 cwd、storage path 或可复用 capability。
