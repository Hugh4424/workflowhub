...
- [ ] T001 改造 `core/task-dir-parser.mjs`：新增 `WORKFLOWHUB_TASK_DIR` env var 优先级（① env var → ② yaml fallback → ③ 两者缺失 fail-loud），parser 返回 task_tracking_root 本身（不拼接 `/tasks/{task-id}`）；路径不存在或非目录时 fail-loud；保留 yaml fallback（D1 不破坏）；删除旧硬编码路径 `~/Knowledge/workflowhub/`（FR-WORKTREE-ENVVAR-003 第 3 条：两者均缺失时 fail-loud，不使用硬编码）。**顺带新建 `core/worktree-context.mjs`**（同为 core/ 共享工具脚本）：接口 `node core/worktree-context.mjs <worktree.json路径>`，字段完整则打印 JSON 到 stdout 并 exit 0；`target_repo_root`/`worktree_root` 字段缺失则 stderr 报错并 exit 1。FR: FR-WORKTREE-ENVVAR-003, AC-16 (stage:1, depends:无) (upstream_delta: research.md §3 — core/task-dir-parser.mjs 硬编码路径，scripts/ 为空无辅助脚本)
  - **gate_cmd**:
...
- [ ] T005 核查并按需最小修改 `workflows/build-spec/SKILL.md` 和 `workflows/build-plan/SKILL.md`（FR-WORKTREE-SCOPE-008 要求覆盖二者）：（1）确认两者均不执行 `git worktree add`（不新增 worktree 条目）；（2）确认各自已包含读取 `worktree.json` 中 `target_repo_root`/`worktree_root` 字段的逻辑，以及字段缺失时 fail-loud 报错退出的说明；若尚未存在，**仅允许新增该段最小实现**（读取字段 + 缺失 fail-loud），其余逻辑一律禁止修改；若已存在则记录"无需修改"并跳过；核查/修改结论写入 stage-result / journal。**新增交付物**：`core/worktree-context.mjs` — 共享脚本，build-spec/SKILL.md 和 build-plan/SKILL.md 都必须通过它读取 worktree.json 的 `target_repo_root`/`worktree_root` 字段（不允许各自重复实现读取逻辑）。该脚本接口：`node core/worktree-context.mjs <worktree.json路径>`，字段完整则打印 JSON 到 stdout 并 exit 0；字段缺失则 stderr 报错并 exit 1。build-spec/SKILL.md 和 build-plan/SKILL.md 须新增/确认调用 `core/worktree-context.mjs` 读取字段（而非各自 inline 重复实现），其余逻辑禁止修改。FR: FR-WORKTREE-SCOPE-008 (stage:3, depends:T001,T002,T003,T004) Knowledge: 先 Read build-spec/SKILL.md 和 build-plan/SKILL.md，确认现有内容后决定是否需要最小补充
  - **交付物**：`core/worktree-context.mjs`（新文件，build-code 阶段创建）
  - **gate_cmd**:
    ...
    - build-spec 真实引用 core/worktree-context.mjs：`grep -q "worktree-context\.mjs\|core/worktree-context" workflows/build-spec/SKILL.md` — exit 0（**blocking**：build-spec/SKILL.md 须真实引用/调用 core/worktree-context.mjs，不得仅有字段名；缺失则 exit 1，阻断）
    - build-plan 真实引用 core/worktree-context.mjs：`grep -q "worktree-context\.mjs\|core/worktree-context" workflows/build-plan/SKILL.md` — exit 0（**blocking**：build-plan/SKILL.md 须真实引用/调用 core/worktree-context.mjs，不得仅有字段名；缺失则 exit 1，阻断）
    ...
    - core/worktree-context.mjs 存在：`_ROOT=$(git rev-parse --show-toplevel) && test -f "$_ROOT/core/worktree-context.mjs"` — exit 0（**blocking**：真实交付物必须存在；缺失则 exit 1，阻断）
    - 构造缺失字段场景验证 fail-loud（真实脚本）：`_ROOT=$(git rev-parse --show-toplevel) && _F=$(mktemp --suffix=.json) && printf '{"branch":"workflowhub/test","status":"active"}' > "$_F" && node "$_ROOT/core/worktree-context.mjs" "$_F" ; _rc=$? ; rm -f "$_F" ; test $_rc -ne 0` — exit 0（**blocking**：缺 target_repo_root/worktree_root 时须退出码非零）
    - 构造字段齐全场景验证 exit 0（真实脚本）：`_ROOT=$(git rev-parse --show-toplevel) && _F=$(mktemp --suffix=.json) && printf '{"target_repo_root":"/tmp/repo","worktree_root":"/tmp/wt","branch":"workflowhub/test","status":"active","created_by_stage":"make-decision","push_policy":"verify-code-only"}' > "$_F" && node "$_ROOT/core/worktree-context.mjs" "$_F" ; _rc=$? ; rm -f "$_F" ; test $_rc -eq 0` — exit 0（**blocking**：字段齐全时须退出码 0 且打印 JSON）
...
- [ ] T008 验证 FR-WORKTREE-COMMIT-004 per-phase commit 规则在各阶段 SKILL.md 中落地，覆盖 make-decision、build-code、build-spec、build-plan、verify-code 和 close 归档 commit 共 6 个触发点：
  - **T008-F（make-decision commit 触发点）**：make-decision/SKILL.md 须包含当阶段目标仓库有文件变更时须 commit 的说明（commit message 含 `workflowhub(make-decision)` 前缀）及无变更时须写 stage-result.missing_items 或 journal 记录的说明；缺失则阻断。
  ...
  - **T008-D（verify-code close 归档 commit 触发点）**：verify-code/SKILL.md 须包含 close 阶段归档 commit 说明，commit message 精确为 `workflowhub(close): archive {task-id}`（不得使用 `workflowhub(verify-code)` 前缀，close 不是独立 stage）；缺失则阻断。
  ...
  - **gate_cmd**:
    - T008-F make-decision commit 规则存在：`grep -q "workflowhub(make-decision)\|commit.*make-decision\|make-decision.*commit" workflows/make-decision/SKILL.md` — exit 0（缺失则 exit 1，阻断）
    - T008-F make-decision no-change 记录规则存在：`grep -q "missing_items\|journal" workflows/make-decision/SKILL.md` — exit 0（缺失则 exit 1，阻断）
    ...
    - T008-D verify-code close 归档 commit 前缀正确：`grep -q "workflowhub(close): archive" workflows/verify-code/SKILL.md` — exit 0（缺失则 exit 1，阻断）
