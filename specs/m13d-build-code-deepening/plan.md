...
#### 2.2: L2 集成冒烟章节
描述：所有 phase GREEN 后，调用 agenthub 仓库的 `test-routing-advisor` 技能生成 `l2-integration-test-report.json`（routing_tier + result + ts），档位选择理由需可追溯写入报告；冒烟失败不阻断 build-code。
...
描述：verdict-handler 监控同一 phase/stage 同一审查子代理连续 `revise_required` 次数，达 3 次触发 C 类升级（escalate_to_human），升级时产出结构化记录。
**Files**: `workflows/build-code/SKILL.md`
**Maps to**: FR-REVIEW-002
...
描述：build-code 阶段确认 `tasks/{task-id}/worktree.json` 存在即直接复用（不重复拉取），不存在时按 make-decision 阶段规则自建；本任务自身已验证复用成功（`tasks/m13d-build-code-deepening/worktree.json`）。
**Files**: `workflows/build-code/SKILL.md`（读取协议说明，不改变已生效的 worktree_root 配置机制）
**Maps to**: FR-WORKTREE-001
...
| 2.4 verdict-handler A/B/C | FR-REVIEW-002 | AC-REVIEW-004, AC-REVIEW-005 |
...
| 3.2 单测补充 | 跨 FR | AC-RISK-001, AC-SMOKE-001, AC-METRICS-001~003 |
