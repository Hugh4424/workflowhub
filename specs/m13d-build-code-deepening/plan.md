...
#### 2.2: L2 集成冒烟章节
描述：所有 phase GREEN 后，调用 agenthub 仓库的 `test-routing-advisor` 技能生成 `l2-integration-test-report.json`（**routing_tier + routing_rationale（非空字符串，档位选择理由）+ result + ts**）；冒烟失败不阻断 build-code。（独立审查 round2/round3/round4 发现：此前本节仅写 routing_tier+result+ts、"理由需可追溯"未落成具体字段名；字段名 routing_rationale 由 data-contracts.md/tasks.md T010 定义——T004 本身只要求理由可追溯、不点名字段，现已对齐 data-contracts.md/T010 的定义，不归因给 T004）
