...
读取失败为 `unavailable`；身份不足为 `unknown`。计划、步骤记录、完成结果分开；确认先于
逐动作授权；失败记录不覆盖，完成结果只在全部物理项读回满足后产生且同一计划唯一。
...
**任务类型（task type）**：
每个任务自己的显式属性，当前只定义 `规划任务` 与 `普通任务` 两种行为类型；类型声明写在当前
`specs/<task-id>/decision-log.md` 的任务身份段，make-decision 开始提问前必须可唯一读出。
缺少声明时按 `unknown` 如实报告，不得推断；调研、可行性测试、设计等其他类型只保留位置，行为不变。

**规划任务提问边界（planning-task question boundary）**：
规划任务只询问会改变方向的问题：用户可见行为与结果、任务类型范围、工作流程、完成与失败条件、不变量、约束、非目标和延期；不询问文件路径与文件面、函数名、字段名、算法、schema 形状、命令形态、入口参数形态、行号、代码片段、测试记录与实测记录。
普通任务保留原有提问强度。该边界是任务类型维度的架构选择，对应 ADR 0027。
...
本节记录当前规划方向；上文五阶段与执行记录仍描述现行系统，不因本节而失效。方向来源：specs/workflowhub-thin-core-rebuild-planning-20260919/decision-log.md 的U-007/T-006；职责取舍见ADR-0031，具体方案纳入该任务最终确认。

**方法工具包**：由现有AI助手及子代理承担执行，WorkflowHub提供可组合工作方法与必要窄工具、不提供逐阶段强制认证的目标产品形态。

**可验证用户结果工作包**：围绕一项可观察用户结果或支撑该结果的明确基础能力，具有清楚输入、写入范围和验证边界的工作组织单元。
