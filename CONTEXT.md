...
**host-independent WorkflowHub core / platform adapter**：
WorkflowHub 五阶段 runtime、task storage、StageContext、skills 与通用 contracts 可在没有 Multica 时独立运行。Multica 只是可选 platform adapter；offline fixture 或其他 adapter 经同一 canonical port 接入。adapter 不进入 core 业务裁决。既有文档中的 generic core / Multica adapter 是这一边界的旧称。
