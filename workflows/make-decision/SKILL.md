...
3. **致命输入停止**：S3（`get_sources` 无法核实）和 S5（关键前置文件缺失或 `source_family` 冲突且无法降级）在输入不可用时按 let-it-crash 原则停下报错——属于"输入不可用的致命停止"，不是质量门，不同于推进硬门。
...
## S2 talk#1（呈现内部调研摘要，问 Q1）

**目标**：向用户呈现内部调研摘要，收集"是否需要外部双路调研"的判断，完成第一轮 talk。

**三轮结构（talk#1 / talk#2 / talk#3）**：本阶段为第一轮（Q1），后续 S4 完成 Q2，S7 完成 Q3。

**执行步骤**：

1. 先向用户呈现 `tasks/{task-id}/research/internal-research-summary.md` 内容摘要
   （lite 档时以"内部调研已跳过（scope=lite）"说明）。
2. 按影响排序（FR-TALK-02），一次只问一个问题（FR-TALK-01）：
   - **lite 档**：跳过 Q1（不询问外部调研需否）——因为 lite 档的 S3 无论如何均跳过，Q1 无意义。直接记录 `s3: skipped: scope=lite`，进入步骤 3 的后续问题（如有）或结束 talk#1。
   - **full 档**：问 **Q1**：是否需要外部双路调研（muyu-search-mcp + anysearch）？
3. 等待用户回答（仅 full 档）：
   - 用户确认需要 → 继续 S3 外部调研。
   - 用户跳过或不需要 → 记录 `s3: skipped(user_decision)`，直接进 S4。
...
## S4 方向收敛 talk#2（台账渲染点①）

**目标**：向用户展示调研汇总与初步方向，通过 Q2 收敛方向；完成台账渲染点①。

**执行步骤**：

1. 汇总 S1/S3 调研产出（或空/跳过说明），向用户展示方向摘要。
2. 按影响排序，**一次只问一个问题**（FR-TALK-01/02）：
   **Q2**：是否认同当前方向草案？有无关键约束或范围需调整？
...
并行启动三条独立 3rd-review 链，**三个 agent 输入互不可见（隔离）**，彼此不共享对方的审查输入或中间结论：

- `intake-direction-review`：审查方向合理性
- `intake-framing-challenge`：挑战问题框架设定
- `intake-scope-review`：审查范围边界合理性
...
source_family: <来源族，三个 source_family 两两不同>
fallback_used: <true|false>
input_hash: <该角度输入内容的哈希，用于隔离验证>
```

### 2. 合并条件与失败语义

- 若某角度 `source_family` 与另一角度相同（同源降级）→ 立即记录该角度 `fallback_used: true`，视为该角度审查失败，结果不进合并，立即停下报告用户，**禁止静默降级**。（`source_family` 冲突与 `fallback_used: true` 是同一语义事件，非两个独立门。）
- 三个角度全部通过后合并产出：
  - `direction_divergence`: `true`/`false`（方向分歧标记）
  - `findings`: 各角度 findings 列表
...
3. 其余情况：提取三个审查 artifact（`make-decision-review-direction.md`、`make-decision-review-framing.md`、`make-decision-review-scope.md`）中具体的 finding ID 列表（争点来源，可为空），传入 debate 技能 + Claude 决策 + decision-log 版本，由 debate 技能的 Step 1 自行判断是否触发；读回 debate 技能产出的裁决书。
...
**目标**：独立展示步骤（独立展示步骤），向用户呈现 S5 三角度盲审结果与 debate 裁决。

**执行步骤**：

1. 展示三角度盲审 findings 摘要（三角度盲审 findings 摘要），包含每个角度的核心 findings。
...
## S7 talk#3 → grill → draft → orchestrator → 第二次 debate
...
- 综合 S5 三角度 findings 与 S6 展示结论，按影响排序（按影响排序）生成 Q3 追问清单（Q3）。
- 一次只问一个问题（FR-TALK-01/02），等待用户回答。

写 journal 事件：`event: "s7_talk3_done"`

### 2. grill（纯委托 grill-with-docs-lite）

退出条件：用户能复述四件事——做了什么 / 为何 / 不做什么 / 怎么验证。

纯委托（pure delegation）给 `grill-with-docs-lite` skill 执行，退出逻辑由其内部控制，不在本 agent 内联展开。
...
- 若存在 `severity: blocking` 的审查意见，但对应审查产物（`make-decision-review-direction.md` / `make-decision-review-framing.md` / `make-decision-review-scope.md` 及 debate 裁决书）中**缺少**三行留痕格式：
  ```
  反对 X：<内容>
  决定 Y：<内容>
  理由 Z：<内容>
...
**执行环境**字段（小节，写在 7 节之后）：记录本次执行中 6 个 env var 的检测结果，包含：
...
### 3. 调用 metrics/collector.mjs updateOwnResult
...
| `s5_blind_review_done` | S5 | — | 三角度盲审完成 |
