...
Run this hook before every make-decision step that performs work, review, or checks.

1. Assign a step identifier with format `md.{step_type}.{seq}`:
   - `step_type` must be one of `work`, `review`, or `check`.
   - `seq` starts at `1` and increments within this make-decision run.
   - Example: `md.work.1`, `md.review.1`, `md.check.2`.
...
Run this hook after every make-decision step that performs work, review, or checks.
...
When writing the stage-result, append a top-level `audit_summary` object without changing or removing any existing stage-result fields. Read `journal.jsonl`, parse each JSON line in append order, call `buildAuditSummaryFromJournalEvents(events, { stageSlug: "md", workflowRunId })`, and attach the returned `audit_summary` at the stage-result top level. If `warnings` is non-empty, append the warning strings to an existing `reason` or `notes` field; if neither field exists, create `notes` for the warnings while preserving all existing fields.
...
Populate the summary from entries scoped to the current `workflow_run_id` and the `md.*` local pointer chain only. Start from the first `md.*` entry whose `prev_step_id` is `null` or outside `md.*`; follow `next_step_id` when present, otherwise discover the next entry by matching `prev_step_id` to the current `step_id` in journal append order. Count each distinct `step_id` once. On duplicate links, missing links, or cycles, stop traversal at the malformed edge, record a warning in the stage-result reason/notes, and preserve the partial counts.
...
- `total_step_count`: count of audited make-decision steps observed for this run.
...
- `rollback_count`: count of runner-owned `step_auto_rollback` events for this `workflow_run_id` whose affected `step_id` belongs to the `md.*` local pointer chain.
