...
// artifactPath is the durable path to the review report, resolved via parseTaskDir, e.g. "{taskDir}/{task-id}/reviews/build-code-phase-N.md"
...
At the start of each phase, read the `risk_level` field from `facts.tasks` for that phase. Valid values: `P0` / `P1` / `P2` / `P3` / `null`.

- Record `risk_level` in the phase's GREEN evidence JSON (via `capture.mjs` — see §12 for field details).
- If `risk_level` is `P0`, output a non-blocking warning in the execution log: `[RISK P0] High-risk phase — ensure test coverage exists for this phase.`
- If `risk_level` is absent or `null`, record `null` and continue without warning.
- Do **not** halt or block phase execution based on risk level. Record and proceed (宪法 F3: 记录事实不阻断).

### 12. L2 集成冒烟（FR-SMOKE-001）

After GREEN evidence is collected for any phase where `risk_level` is `P0` or `P1`, run L2 integration smoke if the task type warrants it. Smoke tier selection:

| Tier | When |
|---|---|
| `simple` | Single-file change, no cross-module boundary |
| `feature` | Multi-file change within one module boundary |
| `fullstack` | Cross-module or cross-service boundary |

Produce a smoke report at `{taskDir}/{task-id}/smoke-phase-N.md` containing:
- `result`: `pass` or `fail`
- `tier`: selected tier (`simple` / `feature` / `fullstack`)
- `tier_reason`: one-sentence justification for tier selection
- `timestamp`: ISO-8601

Smoke failure does **not** block phase advancement. Record `fail` in the report and continue (宪法 F3). If smoke tooling is unavailable, record `tier: skipped` and reason, then continue.

### 13. mtime 防伪校验（FR-ANTIFORGERY-001）

Before triggering 3rd-review for any phase, verify the mtime ordering of the three evidence files: `phase-N.md` / `phase-N-RED.json` / `phase-N-GREEN.json`.

Expected order: `phase-N.md` written first, then `RED.json`, then `GREEN.json` (i.e. `mtime(RED) < mtime(GREEN)`).

Violation condition: `mtime(RED.json) > mtime(GREEN.json)` or any other ordering that contradicts "RED before GREEN".

- Check all three files — do not check only one.
- If violation detected: write an escalation record at `{taskDir}/{task-id}/antiforgery-phase-N.md` with the specific violation description (which file, which mtime values), then call `escalate_to_human` and **do not auto-proceed** to 3rd-review.
- If ordering is valid: continue to 3rd-review without escalation (宪法 F3: do not escalate when facts are clean).

### 14. 两阶段 3rd-review 与原子提交（FR-REVIEW-001, FR-COMMIT-001）

**3rd-review 两子代理拆分**

Dispatch 3rd-review as two independent subagents:

1. **review-trigger subagent** — feeds the real `git diff` to the review engine and collects the raw verdict file.
2. **verdict-handler subagent** — reads the verdict file produced by review-trigger, applies the 3-state logic (`pass` / `revise_required` / `escalate_to_human`), and writes the final `facts.review` struct via `buildReviewFact`.

Neither subagent may self-approve. The orchestrator only wires them in sequence; it does not interpret the verdict itself.

If either subagent fails or times out: degrade gracefully — record `source: "same_source"`, `verdict: "unknown"`, reason in `facts.review`, and continue (宪法 Q2: non-blocking).

**原子提交规则（FR-COMMIT-001）**

- Implementation subagents (implementer) **must not** run `git commit` during any phase. Include explicit instruction `DO NOT commit. Leave changes in the working tree.` in every implementer dispatch.
- After all phases are complete and all GREEN evidence + 3rd-review verdicts are recorded, the orchestrator runs a **single atomic commit** covering all phase changes together.
- Commit message must include: task_id, list of changed files, and the phrase `[build-code atomic commit]`.
- If any phase is missing GREEN evidence, do not commit — surface the gap to the user.
