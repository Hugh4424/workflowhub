...
the next review and stage run. If the full-worktree review returns
`revise_required`, repair the same original Phase that owns the finding,
publish a revision receipt, capture fresh tests under new refs, and run a new
full-worktree review. This is append-only repair of the current open stage; it
...
After the final implementation receipt and fresh test receipt exist, run one
independent code review using those fresh tests: one full-worktree `wh-review`
without `phase_id`. Do not publish a separate per-Phase review record or invoke
a separate Phase diff scanner. The review host freezes the authenticated
Workspace itself; do not supply paths, commits, ranges, or a caller-built diff.
...
4. Give the Coder one complete **Coder Phase card** for each Phase, in this
   ...
   allowed files before continuing; this is a local implementation check, not a
   second phase evidence publication path.
...
6. For each Coder handoff, compare the Workspace's changed paths with that
   ...
   return it to the same Phase for correction. Do not publish a per-Phase
   result, receipt, or review subject.
...
8. Run one independent full-worktree `wh-review` without `phase_id` after the
   final implementation receipt and fresh tests. The host freezes the complete
   authenticated Workspace; do not pass paths, commits, ranges, or a
   caller-built diff. Its snapshot tree must equal the canonical implementation
   and tests snapshot tree. On `revise_required`, fix the same original Phase,
   publish an append-only implementation revision receipt, capture fresh tests,
   and repeat the full-worktree review. Do not create a replacement Phase or
   overwrite evidence. If the independent review capability is unavailable,
