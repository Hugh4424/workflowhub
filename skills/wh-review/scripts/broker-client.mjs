...
export class BrokerClient {
  ...
  #materializePrivateRaw(result, privateRawDirectory, rawRequestBinding) {
    ...
    const verifyMaterial = (provider, stateProvider, publicProvider) => {
      ...
      if (!rawRequestBinding || rawRequestBinding.delivery_mode !== delivery.delivery_mode || !Array.isArray(hostRecords)
        || rawRequestBinding.material_manifest_sha256 !== delivery.sealed_manifest_hash) throw new Error(`BROKER_RAW_AUDIT_UNAVAILABLE: ${provider} host request binding is incomplete`);
      ...
      const secureRead = (root, destination) => {
        ...
        try {
          ...
          if (!opened.isFile() || opened.nlink !== 1) throw new Error("workspace destination is not an isolated regular file");
      ...
      const matching = candidates.filter((entry) => {
        try {
          ...
          if (manifest?.manifest_hash !== delivery.sealed_manifest_hash || canonical(manifestFiles) !== canonical(visible)) return false;
    ...
    const copyStream = (provider, stream, stateProvider, publicProvider) => {
      ...
      try {
        const opened = fstatSync(fd);
        if (!opened.isFile() || opened.nlink !== 1) throw new Error(`BROKER_RAW_AUDIT_UNAVAILABLE: ${provider} ${stream} source is not an isolated regular file`);
    ...
    const providers = result.providers.map((item) => {
      ...
      if (item.status === "completed" && item.delivery) verifyMaterial(item.provider, privateState, item);
