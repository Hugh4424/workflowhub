...
let state = { input_hash: inputHash, session_id: null, resume_count: 0, attempt: 0, status: "new" };
...
function atomicWrite(path, value) { mkdirSync(dirname(path), { recursive: true }); const tmp = `${path}.${process.pid}.${Date.now()}.tmp`; writeFileSync(tmp, value); try { renameSync(tmp, path); } catch (error) { try { rmSync(path, { force: true }); renameSync(tmp, path); } catch { rmSync(tmp, { force: true }); throw error; } } }
...
function record(type, meta = {}) { appendFileSync(journal, `${JSON.stringify({ at: new Date().toISOString(), type, attempt: state.attempt, ...meta })}\n`); }
...
async function run(input, resume) {
  state.attempt += 1; persist(resume ? "resuming" : "running"); record("attempt_start", { resume, input_bytes: Buffer.byteLength(input), input_hash: createHash("sha256").update(input).digest("hex") });
  ...
  return new Promise((resolve) => {
    ...
    child.stderr.on("data", (chunk) => record("stderr_activity", { bytes: chunk.length }));
    child.on("error", (error) => { record("spawn_error", { code: error.code }); settle({ error, code: null }); });
    child.on("close", (code, signal) => { consume(buffer); record("child_close", { code, signal, stalled, terminalSeen }); settle({ code, signal, stalled, terminalSeen }); });
...
let resume = Boolean(state.session_id), outcome = await run(resume ? continuation : prompt, resume);
if (outcome.stalled && state.session_id) { state.resume_count += 1; outcome = await run(continuation, true); }
let output;
if (outcome.stalled) output = failure(mode, state.resume_count ? "claude-code-idle-after-resume" : "claude-code-idle-without-session", { session_id: state.session_id, resume_count: state.resume_count });
