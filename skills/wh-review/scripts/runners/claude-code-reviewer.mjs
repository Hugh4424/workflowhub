...
const skillResultsProperty = { type: "array", minItems: requiredSkills.length, maxItems: requiredSkills.length, items: { type: "object", additionalProperties: false, properties: { skill: { enum: requiredSkills }, status: { enum: [...SKILL_STATUSES] }, evidence: { type: "string", minLength: 1 } }, required: ["skill", "status", "evidence"] } };
const schema = { type: "object", additionalProperties: false, properties: { verdict: { enum: [...VERDICTS] }, findings: { type: "array", items: { type: "object", additionalProperties: false, properties: { severity: { enum: [...SEVERITIES] }, file: { type: "string" }, line: { type: "integer", minimum: 1 }, issue: { type: "string" }, recommendation: { type: "string" } }, required: ["severity", "file", "line", "issue", "recommendation"] } }, resolutionSummary: { type: "string" }, skillResults: skillResultsProperty, ...(artifactPackage ? { artifactCoverage: coverageProperty } : {}) }, required: ["verdict", "findings", "resolutionSummary", "skillResults", ...(artifactPackage ? ["artifactCoverage"] : [])] };
const skillInstructions = `Required skills (manifest-authoritative): ${JSON.stringify(requiredSkills)}. skillResults must contain every listed name exactly once and no other names. Each entry needs non-empty evidence. pass/revise_required require status=executed for every skill, except plan-design-review may be not_applicable. Use escalate_to_human when any required skill is unavailable or failed; still return every required skill exactly once with its actual status.`;
