...
describe("invokeReviewEngine — success path", () => {
  ...
  it.skip("legacy canonical Claude chunk transport is retired by V4 packet dispatch", () => {
    ...
    writeFileSync(fakeClaude, `#!/usr/bin/env node
      ...
      const manifest = JSON.parse(readFileSync(join(process.cwd(), "manifest.json"), "utf8"));
      let sequence = 0, readBytes = "";
      for (const entry of manifest.entries) for (const chunk of entry.chunks) {
        const id = "read-" + (++sequence), path = join(process.cwd(), chunk.path), source = readFileSync(path, "utf8");
        readBytes += source;
        // Claude 2.1 emits a numbered terminal empty line when the source ends
        // in LF. Preserve it so the canonical adapter can reconstruct bytes.
        const lines = source === "" ? [] : source.split("\\n").map((line) => line.replace(/\\r$/u, ""));
        const content = lines.map((line, index) => String(index + 1) + "\\t" + line).join("\\n");
        process.stdout.write(JSON.stringify({ type:"assistant", message:{ role:"assistant", content:[{ type:"tool_use", id, name:"Read", input:{ file_path:path } }] } }) + "\\n");
        process.stdout.write(JSON.stringify({ type:"user", message:{ role:"user", content:[{ type:"tool_result", tool_use_id:id, content }] } }) + "\\n");
      }
      writeFileSync(${JSON.stringify(capturedReads)}, readBytes);
    ...
    const canonicalRoot = process.env.THIRD_REVIEW_REPO_ROOT ?? resolve(dirname(fileURLToPath(import.meta.url)), "../../../../../3rd-review");
    const canonicalUrl = pathToFileURL(resolve(canonicalRoot, "scripts/run-heterologous-review.mjs")).href;
    ...
    const canonicalRawPath = join(root, TASK_ID, "reviews", `verdict-${STAGE}-cross-repo-round-1.raw.json`);
    expect(result.verdict, readFileSync(canonicalRawPath, "utf8")).toBe("pass");
    const prompt = readFileSync(capturedPrompt, "utf8");
    expect(prompt).toContain("chunks/002/00001.txt");
    expect(prompt).not.toContain("[... truncated ...]");
    const readBytes = readFileSync(capturedReads, "utf8");
    expect(readBytes).toContain(first);
    expect(readBytes).toContain(last);
    const raw = JSON.parse(readFileSync(canonicalRawPath, "utf8"));
