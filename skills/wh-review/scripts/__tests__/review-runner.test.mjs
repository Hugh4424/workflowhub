...
describe("aggregation and runner", () => {
  ...
  it("does not pass a v2 route when its configured initial quorum is not met", async () => {
    ...
    expect(attempt.provider_attempts.find(({ provider }) => provider === "kimi/k3").output_ref).toContain("provider-a2ltaS9rMw.output.json");
    const reused = await runReviewFixture(options);
    expect(reused).toMatchObject({ status: "unavailable", reused: true, attemptRef: result.attemptRef });
    expect(calls).toEqual(["claude-code/opus", "kimi/k3"]);
  });
