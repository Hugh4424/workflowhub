...
const providerVisibleDestinations = ["unified_diff", "planning_artifacts", "changes.diff", "review-packet.v1.json", "manifest.json", "continuation-delta.v1.json", "evidence/requirements-coverage.json"];
...
function fixture({ stage = "build-code", reviewTrack = null, verdict = "pass" } = {}) {
  ...
  const output = {
    packet_status: "complete", verdict, summary: "审查结论基于冻结 packet 的逐项证据。", findings: [],
    ...
    skillResults: resolution.definitions.map(({ name }) => ({ skill: name, mode: "lens-only", checked_objects: ["planning_artifacts:plan.md#L10"], evidence: "plan.md#L10 显示需求和验证步骤直接关联。", conclusion: "该 lens 未发现违反合同的证据。" })),
  };
...
function validate(value) { return validateReviewerOutput({ providerVisibleDestinations, stage: "build-code", output: value.output, packet: value.packet, intent: value.intent }); }

describe("reviewer-output validator", () => {
  it("validates provider semantics without trusting provider-owned hashes", () => {
    const item = fixture();
    for (const key of ["packet_hash", "manifest_hash", "diff_sha256", "contract_hash", "skill_bundle_hash"]) delete item.output[key];
    expect(validate(item).valid).toBe(true);
  });
  it("requires stable explicit check ids in every selected stage contract", () => {
    ...
    for (const [stage, track, checkCount, hardCount] of cases) {
      ...
      expect(validateReviewerOutput({ providerVisibleDestinations, stage, reviewTrack: track, output: fixture({ stage, reviewTrack: track }).output }).valid).toBe(true);
    }
  });
  ...
  it("accepts a whole provider-visible evidence artifact but rejects unsafe or unknown paths", () => {
    const item = fixture();
    item.output.pass_items.find(({ rule_id }) => rule_id === "H3").artifact_anchor = "evidence/requirements-coverage.json";
    expect(validate(item).valid).toBe(true);
    const continuation = fixture(); continuation.output.pass_items.find(({ rule_id }) => rule_id === "H2").artifact_anchor = "continuation-delta.v1.json";
    expect(validate(continuation).valid).toBe(true);
    for (const anchor of ["bundle/", "/evidence/requirements-coverage.json", "evidence/../requirements-coverage.json", "evidence/result.exe", "unknown.json", ""]) {
      const invalid = fixture(); invalid.output.pass_items.find(({ rule_id }) => rule_id === "H3").artifact_anchor = anchor;
  ...
  it("normalizes punctuation and rejects long combinations of hollow evidence without rejecting anchors", () => {
    ...
    ]) { const skill = fixture({ stage: "build-plan" }); mutate(skill); expect(validateReviewerOutput({ providerVisibleDestinations, stage: "build-plan", output: skill.output }).valid).toBe(false); }
  ...
  it("requires a duplicate-free checklist covering every stage contract check id", () => {
    ...
    expect(validateReviewerOutput({ providerVisibleDestinations, stage: "make-decision", reviewTrack: "direction", output: direction.output, packet: direction.packet, intent: direction.intent }).errors).toContain("checklist missing contract rule id: DIR-C6");
    direction.output.checklist.push({ ...direction.output.checklist[0] });
    expect(validateReviewerOutput({ providerVisibleDestinations, stage: "make-decision", reviewTrack: "direction", output: direction.output, packet: direction.packet, intent: direction.intent }).errors).toContain("duplicate checklist id: DIR-C1");
  });
  ...
  it("accepts only rule ids from the selected make-decision track", () => {
    const direction = fixture({ stage: "make-decision", reviewTrack: "direction" });
    expect(validateReviewerOutput({ providerVisibleDestinations, stage: "make-decision", reviewTrack: "direction", output: direction.output }).valid).toBe(true);
    const detail = fixture({ stage: "make-decision", reviewTrack: "detail" });
    expect(validateReviewerOutput({ providerVisibleDestinations, stage: "make-decision", reviewTrack: "direction", output: detail.output }).errors).toEqual(expect.arrayContaining([
      "checklist id is not in selected contract: DET-C1",
      "pass item rule id is not in selected contract: DET-H1",
    ]));
  });

  it("matches required skillResults exactly without provider-owned hashes", () => {
    const item = fixture({ stage: "build-plan" });
    const call = () => validateReviewerOutput({ providerVisibleDestinations, stage: "build-plan", output: item.output, packet: item.packet, intent: item.intent });
  ...
  it("requires an exact empty skillResults array when build-code declares no lenses", () => {
    ...
    expect(call().valid).toBe(true);
    item.output.skillResults = [{ skill: "build-code-no-extra-lens", mode: "lens-only", checked_objects: ["review-packet.v1.json", "changes.diff"], evidence: "changes.diff covers the whole frozen diff", conclusion: "no extra lens was declared" }];
    expect(call().errors).toContain("unexpected skill result: build-code-no-extra-lens");
  });
});
