...
process.exit(1);`);
  ...
  it("resumes once for the real Claude 524 assistant text frame", async () => {
    const apiError = {
      type: "assistant",
      session_id: "real-524-session",
      message: {
        role: "assistant",
        content: [{
          type: "text",
          text: 'API Error: 524 {"type":"https://developers.cloudflare.com/support/troubleshooting/http-status-codes/cloudflare-5xx-errors/error-524/","title":"Error 524: A timeout occurred","status":524,"detail":"The origin web server did not return a complete response within the 120-second Proxy Read Timeout window.","instance":"REDACTED","error_code":524,"error_name":"origin_response_timeout","error_category":"origin","ray_id":"REDACTED","timestamp":"2026-07-10T13:38:53Z","zone":"example.invalid","cloudflare_error":true,"retryable":true,"retry_after":120,"owner_action_required":true,"what_you_should_do":"Wait and retry.","footer":"This error was generated by Cloudflare on behalf of the website owner."}. This is a server-side issue, usually temporary — try again in a moment. If it persists, check your inference gateway (example.invalid).',
        }],
      },
    };
    const f = fixture(`
...
writeFileSync(new URL("calls.txt",import.meta.url),(resumed?"resume":"initial")+"\\n",{flag:"a"});
if(!resumed){
  console.log(JSON.stringify({type:"system",session_id:"real-524-session"}));
  console.log(${JSON.stringify(JSON.stringify(apiError))});
  process.exit(1);
}
if(process.argv[process.argv.indexOf("--resume")+1]!=="real-524-session") process.exit(52);
console.log(JSON.stringify({type:"result",session_id:"real-524-session",structured_output:${JSON.stringify(verdict)}}));`);
    const result = await execute(f);
    expect(result.output).toMatchObject({ verdict: "pass", resume_count: 1, synthetic: false });
    expect(readFileSync(join(f.root, "calls.txt"), "utf8")).toBe("initial\nresume\n");
    const persisted = `${readFileSync(join(f.stateDir, "journal.ndjson"), "utf8")}\n${readFileSync(join(f.stateDir, "state.json"), "utf8")}\n${JSON.stringify(result.output)}`;
  ...
  it("does not retry ordinary review text that quotes a 524 API error", async () => {
    const f = fixture(`
import {readFileSync,writeFileSync} from "node:fs";
readFileSync(0,"utf8");
writeFileSync(new URL("calls.txt",import.meta.url),(process.argv.includes("--resume")?"resume":"initial")+"\\n",{flag:"a"});
console.log(JSON.stringify({type:"system",session_id:"quoted-524-session"}));
console.log(JSON.stringify({type:"assistant",session_id:"quoted-524-session",message:{role:"assistant",content:[{type:"text",text:'Finding: logs contain API Error: 524 {"error":{"error_name":"origin_response_timeout","retryable":true}} and should be redacted'}]}}));
process.exit(1);`);
    const result = await execute(f);
    expect(result.output).toMatchObject({ failure_reason: "claude-code-non-zero-exit", resume_count: 0 });
    expect(readFileSync(join(f.root, "calls.txt"), "utf8")).toBe("initial\n");
