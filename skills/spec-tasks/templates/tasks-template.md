...
> 基于 accepted spec 和 plan。每个 Phase 是一个完整阅读单元，内部依次说明目标、要做的工作、文件边界、Phase 验收和本 Phase 的全部 task；WorkflowHub 只补版本、边界、DAG 和可执行验证。
...
每一行同时保留 `quality_status` 和 `user_handoff`；这里是阶段摘要，不能替代 task 卡的完成区或质量事实。`tasks.md` 的每张任务卡必须有
`Workflow stage`、`execution_file_paths`、精确命令、实际结果和覆盖限制；路径从
`精确文件`派生，不得创建第二份文件边界。

> **阅读规则**：任务卡只出现在所属 Phase 的 `### Tasks` 下。后面的依赖图和追溯表只是跨
> Phase 索引，不复制任务卡，也不产生第二个完成权威。
...
- 每张 task 卡必须放在它所属 Phase 内；全局依赖图和追溯表只引用 ID，不得把卡片移到 Phase 外或再复制一份。

## Phase [N]：[填写：阶段名称]

> 复制本整段作为下一个 Phase。不要只复制 task 卡；每个 Phase 都要重新写自己的目标、工作、文件、验收、知识、STOP、完成和回滚边界。

### Goal

[填写：从 accepted plan 原样复制本 Phase 的可观察目标。]

- **本 Phase 要做的事情**：[填写：从 plan 提炼的工作范围和顺序；不复制产品 rationale]
- **本 Phase 产出**：[填写：本 Phase 要交给下游的 artifact、行为或证据]
- **本 Phase 不做**：[填写：明确排除项、延期项和不能借本 Phase 扩大的范围]
...
> 以下 task 卡全部属于这个 Phase。一个卡只改变一个行为或一个非行为产物；卡片的 `Phase`、`Workflow stage`、FR/AC、精确文件和完成区必须互相一致。

#### T001 — RED：[填写：失败测试标题]

- **ID**：T001
- **Phase**：[填写：所属 Phase 名称]
- **Workflow stage**：`[填写：make-decision / build-spec / build-plan / build-code / verify-code]`
- **goal**：[填写：一个可观察任务目标]
...
- **execution_file_paths**：`[填写：与精确文件完全相同的 JSON 路径数组，如 ["src/example.mjs"]]`
- **boundary**：files: `[填写：Phase NEW/MODIFY 内的精确路径]`; symbols/regions: [填写：允许修改的 symbol 或 region]
- **输出**：[填写：可交付产物或行为结果]
- **Knowledge**：[填写：最小已核实上下文 / N/A — 理由]
- **test_strategy_owner**：`build-plan/high-intelligence-model`
- **test tier / test method**：`[填写：simple|feature|fullstack；具体测试方法和选择理由]`
- **scenarios / commands / expected exit / oracle**：[填写：成功、失败、状态或 seam 场景；真实命令；预期退出码；可观察判据]
- **fixtures_services**：[填写：fixture、服务、端口、清理责任 / N/A — 理由]
- **browser_route**：[填写：浏览器路由和隔离 QA 方式 / N/A — 理由]
...
- **execution_contract**：[填写：build-code 如何核对真实 changed files、必要时如何重路由 / N/A — non-behavior change: 理由]
- **TaskKernel/evidence_refs**：[填写：实际命令、exit、snapshot 和 canonical receipt 的绑定方式]
- **evidence_note**：[填写：证据如何证明 oracle，不把 display_cmd 当判据]
...
- **Workflow stage**：`[填写：与 T001 相同的 stage]`
...
- **test_strategy_owner**：`build-plan/high-intelligence-model`
- **test tier / test method**：`[填写：simple|feature|fullstack；具体测试方法和选择理由]`
- **scenarios / commands / expected exit / oracle**：[填写：与 T001 相同的行为场景、真实命令、预期退出码和 ORACLE-ID]
- **fixtures_services**：[填写：与 T001 相同的 fixture/服务和清理责任 / N/A — 理由]
- **browser_route**：[填写：与 T001 相同的浏览器路由 / N/A — 理由]
...
- **coverage limits**：[填写：本命令覆盖范围和明确未覆盖项]
- **execution_contract**：[填写：build-code 如何核对真实 changed files、必要时如何重路由]
- **TaskKernel/evidence_refs**：[填写：实际命令、exit、snapshot 和 canonical receipt 的绑定方式]
- **evidence_note**：[填写：证据如何证明 GREEN oracle 和负例仍受保护]
...
> 这是本 Phase 的整体验收区：它检查 Phase 目标、产出、AC 和跨 task seam；不要把它改成某一张 task 卡的重复完成区。
...
- **Acceptance items**：
  ...
  | --- | --- | --- | --- |
  | `[填写：AC ID]` | `[填写：成功、失败或 seam 场景]` | `[填写：可观察判据]` | `[填写：task-relative ref/hash]` |

- **Task coverage**：[填写：本 Phase 的全部 T-ID → AC ID；不得遗漏 plan 中的 task]
- **Cross-task seam**：[填写：跨 task 的交互、顺序或共享产物验收 / N/A — no cross-task seam: 理由]
- **gate_cmd**：[填写：已核实可执行命令]
...
> 这是跨 Phase 的依赖索引。权威 task 内容仍在各自 Phase 的 `### Tasks` 下；不要在这里复制 task 卡。
...
- [ ] 每个 Phase 八段完整，且 Goal 中写清本 Phase 要做的事情、产出和不做项；Files 与 plan 逐字一致。
- [ ] 每个 Phase 的 Verify 明确 Phase acceptance，并且所有 task 卡物理上位于该 Phase 的 Tasks 区。
- [ ] 每条 Phase acceptance 都绑定 AC、场景、oracle 和 evidence；Task coverage 与 accepted plan 的 Phase task 集合一致。
