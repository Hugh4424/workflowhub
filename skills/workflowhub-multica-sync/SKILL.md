...
3. `skills/catalog.yaml` 中状态为 `native`、`adopted`、`adapted` 的本地技能及其 `skill-bundle.json` 配套文件。
4. 七个核心 agent：工头、Decision Maker、Spec Builder、Plan Builder、Code Builder、Code Verifier、Coder 的 Multica 提示词和技能绑定。

外部技能（例如 `anysearch`、`caveman`）只检查是否被使用或绑定，不从网络或不固定的 `main/latest` 自动覆盖。
...
Multica CLI 无法连接、workspace 不明确或配套文件读取不完整时，状态必须是“无法确认”，立即停止；不能把未读取到的数据当成一致，也不能自动换 workspace、profile 或 provider。
...
1. 重新执行一次审计；如果 `main`、本地技能 bytes、agent 当前版本或 Multica skill IDs 已变化，停止并先报告快照变化。
2. 只同步本地 `catalog.yaml` 管理的技能；保留 Multica skill ID 和已有 agent 绑定。
3. 本地技能缺失于 Multica 时创建；已有技能只更新正文和 bundle 声明的配套文件。
...
8. 每次 mutation 后立即回读；失败立即停止，不自动换 profile、workspace、runtime 或 provider。
