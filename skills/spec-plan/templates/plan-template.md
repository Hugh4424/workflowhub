...
> 这是 plan.md 的阶段进度索引，不复制 decision-log/spec；质量状态和用户看过状态必须如实填写。

| Stage | Status | Work / artifacts | Review / handoff | Next / deferred risk |
| --- | --- | --- | --- | --- |
| make-decision | `[填写：in_progress / completed / unknown]` | `[填写：决策范围和 refs]` | `[填写：review fact；用户看过状态]` | `[填写：build-spec 或未决项]` |
| build-spec | `[填写：in_progress / completed / unknown]` | `[填写：FR/AC 和 revision ref]` | `[填写：review fact；用户看过状态]` | `[填写：build-plan 或风险]` |
| build-plan | `[填写：in_progress / completed / unknown]` | `[填写：Phase/Task 和 refs]` | `[填写：review fact；用户看过状态]` | `[填写：build-code 或延期项]` |

每行还必须在单元格或紧邻正文中说明：`quality_status`、`artifact_refs`、
`plain_language_summary`、`risks_deferred` 和 `user_handoff`。`pass` 不是推进条件；
缺证据写 `unknown/incomplete`，不能用“材料存在”冒充语义验收。
...
每行同时写 `source_refs`：当前 decision-log 的 `R*`/报告/`INC-*` 和对应 `D*`；再写
FR/Task/AC 映射。来源 ID、FR/AC 和 evidence 必须能双向回放；不能在 plan 中补出没有
决策来源的产品范围。
